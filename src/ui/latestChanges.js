export const LATEST_MAJOR_WORK = {
  ...
  currentStatus: [
    '2026-08-25 19:54 EDT - Simplified the Race Editor portrait navigation by removing the redundant Track root and its Add-only sheet. Track is now the first choice in the Ground mode picker and returns directly to the track workspace, where Add, Move, and the existing selection-contextual node and edge controls remain together; desktop and landscape navigation are unchanged.',
    '2026-08-21 21:25 EDT - Rebuilt VehicleValidationHarness around a coherent authoritative initialization and production-control transaction without changing vehicle physics or calibration. Above-zero-speed cases now atomically synchronize chassis velocity, wheel speed, gearing, shafts, clutch/converter, differential, static suspension/contact state, tire condition, and reset generation; validation rejects initial slip, pending shifts, recovery, backlog, or mixed generations. Manual shifts are adjacent edge-triggered transactions, analog maneuvers traverse the gamepad input path, braking begins at the exact brake-onset pose, acceleration begins at delivered drive torque, and traces use velocity-derived acceleration, effective pressure, physical rack/wheel angles, assist intervention, shift state, and phase markers. Seven exact self-checks gate measured CI shards, targetless required failures render honestly in the report, the bank-transition maneuver starts on a solvable flat approach, missing values/references fail enforcement, GitHub remains baseline-only pending reviewed trace approval, and a separate Chromium Physics Surface benchmark now measures actual browser drawing.',
    ...
    '2026-08-18 15:56 EDT - Tested the stationary-on-bumps hypothesis with the real WRX2 configuration, ContactPatchTireModel, 360 Hz tire path, multi-sample footprints, and a packed 512-triangle road whose four wheel regions differ by more than 15 mm in height and carry distinct normals. The converged immutable hold remains bit-exact for five seconds with zero body, angular, unsprung, compression, and damper velocity; after removing the hold, the ordinary dynamic solver also remains below 2 mm height drift and 0.03 m/s or rad/s body/wheel motion for another five seconds without impact, recovery, or contact-validity changes. Fixed the flat-terrain symmetry incident trigger so legitimate asymmetric loads on bumpy non-coplanar support are not misclassified as a flat-road suspension failure.',
    '2026-08-18 15:44 EDT - Restored visible, stable race-car reset behavior without retuning physics: the immutable stationary hold had thrown away the two valid support confirmations already produced by the converged static iteration and mandatory final contact rebuild, so the first intentional post-reset command appeared ignored. Grounded converged resets now retain those confirmations while still requiring verified support, allowing the next fixed step to perform the existing zero-throttle wake transition. A production Studio Sprint 2 lifecycle regression drives the real delayed reset command from an off-road, inverted, moving WRX2 and proves it visibly returns to packed road geometry on flat, maximum-uphill, and downhill sections, retains at least three-wheel support, and remains position/orientation exact with zero chassis and unsprung velocity for five seconds without impact or route recovery.',
    '2026-08-18 14:48 EDT - Corrected WRX2 left/right wheel alignment without retuning suspension or tires: documented the chassis +X-right convention where negative axle camber leans both wheel tops inward and positive toe-in points both wheels toward the centreline, then introduced one shared axle-to-wheel sign resolver used by suspension geometry, tire contact, and canonical render fallback. Static camber, camber gain, authored toe, and bump steer now mirror on right wheels; physical Ackermann steering remains a separate angle and is no longer serialized as toe-combined steering that the renderer would add toe to twice. Camber thrust consumes the signed physical wheel camber. Exact WRX2 tests verify authored alignment signs, renderer/physics agreement, independent Ackermann, and paired centerline-mirrored zero-steer behavior while settled, coasting, accelerating, braking, and holding 30/60 mph, including lateral/yaw bias, axle loads, wheel-hop symmetry, and mirrored tire forces.',
    '2026-08-18 14:39 EDT - Eliminated stationary-reset wheel oscillation without changing WRX2 tuning: every converged reset now creates a deeply immutable StaticVehicleSupportState containing the body, four suspensions, loads, hubs, mounts, contact planes/triangles, footprint data, validity, terrain revision, and reset generation. Parked steps project that exact transaction with zero wheel dynamics instead of running and copying the ordinary tire solver; lightweight terrain/revision/penetration validation schedules an atomic fresh-frame static rebuild when support genuinely changes, while wake restores the immutable transaction, suppresses throttle for the transition step, and uses a 0.25 mm continuous-plane target deadband. Static equilibrium moment correction now transforms world residuals into body axes, removing yaw-dependent left/right support bias. The real WRX2 packed-terrain regression passes both coplanar diagonal splits at 0/90/180/270 degrees and 120/240/360 Hz through five parked seconds and a 20% throttle wake; exact hold, wake, and mutable sample-ownership tests also pass under 192 MB.',
    '2026-08-18 13:22 EDT - Replaced dense body/static probe constraints with bounded persistent manifolds while retaining the full candidate and CCD detection pass: contacts are clustered by collider/surface, physical feature, connected compound piece, and normal direction, then reduced to the deepest and tangent-extreme representatives with at most four constraints per cluster. Restitution is calculated once from the new cluster pre-impact normal velocity and suppressed for preceding-step persistence, resting/slow contact, and suspension-supported underbody contact; friction now accumulates against the cluster representatives normal impulse rather than probe count. Static CCD detects recurring zero-time manifold signatures, removes only inward velocity, preserves tangent travel, and terminates the remainder cleanly instead of replaying sixteen impacts. Multi-normal static overlap uses bounded minimum-translation/minimum-rotation constraint projection instead of chasing one deepest normal, and a deterministic shallow-contact stuck detector schedules a coherent 2 mm separating rebuild under reverse/steering input without route recovery. Focused dense-face, low-speed bumper, head-on/glancing wall, persistent reverse, wall/floor corner, tunnelling, and exact 300 mph robustness checks pass.',
    '2026-08-18 13:07 EDT - Made each collision substep commit one coherent chassis, suspension, wheel, and tread-contact transaction without changing handling: body or static correction above 1 mm or 0.05 degrees invalidates the initial tire result, algebraically rebuilds all four suspension coordinates, axes, hubs, footprints, loads, forces, support validity, and tire contacts from the corrected pose, then revalidates collision for at most three coupled passes against the same prepared chassis-step query frame. Every committed body, suspension, and contact record carries one contactTransactionGeneration; the former compression-only projection repair is removed. Transaction telemetry separates kinetic and mechanical energy around collision, correction, rebuild, and final commit, while non-swept cumulative correction above 6 cm is classified as catastrophic solver failure instead of silently translating a live vehicle. A focused correction regression verifies complete wheel rebuilding, generation coherence, energy-neutral pose correction, and no gameplay reset.',
    '2026-08-18 12:38 EDT - Replaced the post-crash dt=0 dynamic tire loop with an explicit deterministic static vehicle-support solve using the real WRX2 suspension geometry, tire stiffness, bump/hard stops, body clearance, and prepared terrain. Each candidate now rebuilds algebraic wheel compression from its current body pose, balances complete world force and CG moment residuals through damped heave/pitch/roll corrections, and performs a final full contact rebuild before commit; failed candidates are rejected in favor of backward route recovery candidates, with an explicit stopped hard failure if none converge. Reset holds are created only from converged ground support, accept stable three- or two-wheel support, bound contact confirmation time, and never suppress throttle indefinitely. A bounded incident record retains crash/request/installed poses, every equilibrium iteration, residuals, wheel state, and the first 60 fixed steps; exact flat, 10%/20% slope, bank, crest, three-wheel, impact/rollover/jump, and recorded Studio Sprint 2 crash regressions use the real ContactPatchTireModel under the Termux memory cap.',
    '2026-08-18 10:58 EDT - Fixed route-center reset unexpectedly returning from race playtest to the level editor: the completed authoritative reset transaction was scoped inside its try block, then referenced outside it, raising a ReferenceError that the race safety handler correctly treated as a simulation failure and used to tear down playtest. The reset result now remains in scope for atomic render-state installation; an exact regression verifies playtest stays running, the same authoritative runner remains active, and its yaw matches the track pose through two parked seconds and staged wake.',
    '2026-08-18 10:29 EDT - Eliminated reset-to-driving suspension and collision energy discontinuities without retuning vehicle physics: authoritative reset now runs a deterministic bounded static-equilibrium loop over contact geometry, suspension/tire compression, wheel loads, body height, support normal, bump state, and underbody clearance, with explicit convergence/failure diagnostics. The settled state clears every velocity, slip, queued impulse, impact, and recovery source; StationaryResetHold retains complete valid contacts and refreshes them when terrain changes. Wake now requires two consecutive valid contact confirmations, releases the pose constraint on a zero-throttle transition step, then preserves the same springs, loads, and patches for powered integration. A bounded first-60-step record captures generation, body/suspension motion, loads, impulses, corrections, energy, events, and presentation-source labels, while worker reset acknowledgements carry the matching equilibrium result. Exact flat, 10%/20% slope, uneven, post-crash, rollover, and jump reset timelines pass with no held drift, impact, recovery, or load discontinuity.',
    '2026-08-18 08:46 EDT - Made authoritative vehicle resets atomic full-body-and-wheel presentation transactions without changing physics tuning: every reset now advances one vehicleResetGeneration shared by physics state, canonical VehicleRenderState, all four wheel records, worker snapshots, provisional presentation, debug rigs, contact annotations, and reset acknowledgement. Inline reset installs the returned canonical state immediately; worker reset retains a coherent four-wheel provisional state, rejects older generations, and installs a complete compact reset snapshot before clearing pending reset state. Missing or unsupported contacts no longer remove wheel geometry, and Physics Vehicle Rig always draws four finite attached cylinders/hubs/axes with green supported, yellow proximity, red unsupported, or grey terrain-unavailable state. Focused authoritative reset, VehicleRenderState, protocol, delayed snapshot ordering, worker acknowledgement, and bridge transaction tests pass under the Termux memory cap.',
    '2026-08-17 23:19 EDT - Hardened VehicleDynamicsWorker presentation against slow render and debug frames without changing physics: SharedArrayBuffer snapshot slots now use atomic publication sequence counters and latest-wins overwrite semantics, while the render client rejects overtaken or torn slot notifications and retains a bounded four-snapshot ordered history. Presentation timing uses a smoothed measured worker interval, caps coherent body/angular-velocity extrapolation at 33 ms, holds the complete reconstructed vehicle after that cap, and follows angular velocity through quaternion sign changes and rotations beyond 180 degrees. Delivery telemetry now includes latest and displayed sequences, age, interval, drops, overwrites, interpolation delay, extrapolation duration, and buffer starvation; focused 16/33/100/250 ms stall regressions cover ring wraparound, bounded storage, attached wheels, and forward rotation.',
    '2026-08-17 23:14 EDT - Finished worker-authoritative Track State debug synchronization and camera-safe Physics Surface compositing: bounded 10 Hz deltas now carry step, visual revision, event sequence, dirty atlas tiles, and compact visual cells, with atomic cache replacement at migration, worker fallback, reset, and route recovery. Debug lines and triangles transform to camera space, reject or clip against the near plane before positive-depth projection, then clip to the playtest viewport; mobile rendering keeps camera-region distance/triangle/collider budgets and no longer draws a complete terrain wireframe. The compositor now draws Track State colours, terrain/contact lines, the transparent Three.js physics rig pass, then screen-space legend/telemetry, while the HUD separately reports sync, atlas, culling, clipping, projection, path, and canvas p95 costs.',
    '2026-08-17 23:03 EDT - Completed the canonical VehicleRenderState boundary and restored a true Three.js physics vehicle rig without changing handling: render-thread, worker, player, AI, camera, and debug consumers now share body-local hubs, mounts, suspension axes, compression, physical steering, wrapped spin, camber, toe, contact flags, and debug world contacts through worker protocol v4. Physics Surface and Physics Vehicle Rig modes hide the authored sprite and render the compound collider, four wheel cylinders, mounts, axes, hubs, tread contacts, normals, CG, and colored body axes under the authoritative body quaternion rather than billboarding toward the camera. Focused hill, bank, compression, collision correction, CCD rollback, recovery, spin-wrap, protocol, bridge, and sub-centimeter hill-handoff regressions pass under the Termux memory cap.',
    '2026-08-17 22:39 EDT - Restored the inline race-feel diagnostic boundary after canonical VehicleRenderState adoption: render-facing vehicle3d wheels remain schema-identical to worker snapshots, while active render-thread physics keeps its richer contact diagnostics in the dedicated wheelContacts3d compatibility channel. Authoritative physics consumers again prefer the live non-dormant runner, preserving friction-circle and load-sensitivity reporting without changing tire forces or grip; the exact 1200 HP AWD and AWD/RWD/FWD dirt regressions pass.',
    '2026-08-17 20:37 EDT - Synchronized worker-owned Dynamic Track State presentation through bounded, revisioned visual-cell deltas at the 10 Hz Track State boundary, so migration no longer leaves the render thread displaying its dormant dirt, snow, water, or rubber state. Physics Surface debugging now clips world lines and polygons against the camera near plane before projection, clips drawing to the playtest viewport, rejects behind-camera and non-finite geometry, and renders world surface, contacts, vehicle, then screen-space legend in explicit layers. Mobile debug rendering uses camera-region caches, a 120 m distance limit, frustum culling, bounded triangles/colliders, batched body edges, and reusable atlas/canvas pixel storage; the performance HUD reports separate atlas, culling, projection, path, and canvas p95 costs.',
    '2026-08-17 17:05 EDT - Unified render-thread and worker vehicle presentation behind one versioned VehicleRenderState built only after the authoritative fixed step completes collision correction and recovery: body pose and motion now carry four body-local hubs, suspension mounts/axes, compression, physical steering, continuous wheel spin, contact/load flags, and debug-only world contact geometry through the same schema. Interpolation delays presentation behind a four-snapshot history, reconstructs wheel world poses from the interpolated chassis instead of blending independent world wheels, preserves spin direction across angle wrapping, and caps optional velocity extrapolation at 33 ms. Shared-buffer latest-state delivery keeps publishing through render stalls, with a bounded transferable fallback; handoff marks old runners dormant and exposes snapshot age/interval/drop, interpolation/extrapolation, body-angle, and per-wheel local-offset diagnostics. Automatic worker migration is temporarily blocked on mobile unless explicitly forced for debugging.',
    '2026-08-17 14:39 EDT - Revalidated and activated the dedicated VehicleDynamicsRunner worker boundary without using it to mask slow physics: normal races now first collect a live single-thread qualification window, and migrate automatically only when measured throughput exceeds 120 steps/s, p95 stays within the 8.33 ms fixed-step budget, and backlog never grows; missing or failing evidence retains render-thread authority, while forced development activation also fails closed without an explicit passing qualification. The worker owns player/active-AI clocks, collision, tire/suspension state, replay inputs, packed terrain, and deterministic Track State mutation; render exchange remains bounded to compact transferable control/environment/reset buffers and two recycled 548-byte authoritative snapshots with interpolation and independent worker/render p50/p95/p99 metrics. The current post-rollover Android capture measures 338.59 steps/s with 2.18/5.45/8.55 ms step p50/p95/p99 and zero backlog; ownership/protocol/migration tests and the isolated 80-collider responsiveness run sustain 120 Hz with zero terminal backlog.',
    '2026-08-13 13:08 EDT - Reverted the premature default vehicle-worker rollout after live play exposed the exact presentation regressions its opt-in gate was intended to prevent: flipped cars could display wheel snapshots that did not rotate coherently with the body, and asynchronous reset presentation could reintroduce a visible jump. Normal races again keep the proven render-thread authority that publishes chassis and all four authoritative hubs in one synchronization boundary; the worker remains available only through explicit forced testing until equivalent live crash/reset parity is demonstrated. The independent 100 ms Track State workload cap and stale-input reset fix remain active, and default-authority, explicit worker, inverted-hub, reset, and migration regressions pass.',
    '2026-08-13 13:01 EDT - Reproduced and fixed the WRX2 / Studio Sprint 2 post-crash reset corruption and late-race hitching with the exact saved car and 56,448-triangle race: a six-impact first-hill crash left a full-throttle render sample 0.2 nanoseconds beyond the fixed-step reset timestamp, so the first zero-input frame interpolated toward stale crash throttle, released incline parking, and restarted the body/suspension collision oscillator. Authoritative reset now discards all reset-time/future controls before inserting its neutral sample, retains only true thermal/wear/damage tire state instead of crash-frame hub/contact geometry, ignores zero-magnitude collision impulses, and keeps parking through any verified reset-pose recovery. Dynamic Track State background evolution is capped at 64 exact-on-access cells per 100 ms boundary, cutting the measured periodic surcharge from 4.3 ms to 1.6 ms. The exact 100-second render-authority capture holds 408 cells/713 events with a 16.9 ms final-window p95 instead of progressively degrading; exact crash/reset, passive jump, incline, maximum-grade, Track State, worker reset/protocol, and migration regressions pass.',
    '2026-08-12 20:09 EDT - Fixed the remaining one-wheel offset visible immediately after a worker-owned hill reset: RaceEditor moved route/body fields and submitted the asynchronous worker reset while the previous worker presentation snapshot stayed visible until a later update, so Physics Debug could briefly combine the reset context with old wheel geometry. Reset submission now immediately rigid-transforms the complete displayed chassis, four hubs, mounts, axes, contacts, and normals into the new pose and clears the stale presentation; the worker contact rebuild replaces that provisional assembly atomically. A body-local offset regression proves no individual wheel can shift before acceleration, while the exact Studio Sprint 2 jump and six-second maximum-grade launch remain stable.',
    '2026-08-12 19:22 EDT - Removed the hacky post-integration low-speed chassis/wheel velocity scaling that made WRX2 road motion feel disconnected and could increase driving bounce. The underlying stiff tire/unsprung oscillator now dissipates energy inside its force solve through a smooth amplitude-sensitive damping curve: ordinary and hard-impact response retains the authored 1,750 Ns/m tire damping, while only the small-amplitude wheel-hop mode approaches its required damping continuously, with no timer, state threshold, or direct velocity rewrite. Exact first-jump impact behavior, six-second maximum-grade acceleration/braking, 5–20% terrain, rollover hub coherence, worker parity, and suspension-frequency regressions all pass.',
    '2026-08-12 18:19 EDT - Fixed the visible body/wheel disconnect during drift, impacts, and inversion: render-thread synchronization had updated the authoritative chassis every step without copying its four current wheel hubs, mounts, or suspension axes, while the renderer ignored available non-worker hubs and rebuilt contacting wheel centers from ground contact plus surface normal. All authority modes now copy one coherent chassis/wheel snapshot and render the authoritative hub directly, with contact reconstruction retained only for legacy states lacking hub data. Drift/inversion, two-frame 180-degree rollover, worker snapshot, rigid-body, suspension, and exact Studio Sprint 2 regressions verify wheels remain attached without changing physics forces.',
    '2026-08-12 17:29 EDT - Corrected automatic gearing around WRX2 resets and low-speed hill recovery: every automatic route-center reset now selects first gear consistently in RaceEditor, the authoritative runner, and rebuilt powertrain target state instead of retaining the pre-reset high gear, while automatic coasting/downshift logic can continue below 0.5 m/s until first rather than becoming stranded in second or higher at rest. Brake-to-reverse behavior remains intentional and unchanged; focused regressions cover reset from fifth to first, fifth-to-fourth braking, and third-to-first coasting at rest.',
    '2026-08-12 16:46 EDT - Fixed the WRX2 suspension bounce that returned two to three seconds after accelerating from a Studio Sprint 2 maximum-grade respawn: the real RaceEditor 60 FPS path at 35% throttle on the 12.23-degree 220 m grade now runs for six seconds and separately checks sustained acceleration plus braking-to-rest. The expired temporary launch window reproduced renewed 0.114 rad/s rocking; it has been removed and replaced with speed-faded damping of the physically supported chassis pitch/roll and unsprung modes below 8 m/s, keeping all wheels supported during acceleration and settling after braking without changing airborne or higher-speed suspension behavior.',
    '2026-08-12 09:39 EDT - Fixed the remaining WRX2 respawn rocking on Studio Sprint 2 maximum grades: an exact-route scan found the 220 m reset point at 12.23 degrees, where a 6% analog trigger-noise sample previously released parking and reproduced 0.096 rad/s body rocking, 0.238 m/s unsprung motion, and 4.11 m/s downhill drift after six seconds. Reset parking now ignores analog throttle noise through 15% while intentional input releases immediately; exact 4-degree released-settle and 12.23-degree noise-held unit regressions cover both behaviors without changing driving suspension calibration.',
    '2026-08-11 15:42 EDT - Fixed the remaining WRX2 reset oscillation on inclined Studio Sprint 2 terrain: the low-speed suspension-mode detector now recognizes the bounded wheel, heave, pitch, and roll motion produced after an incline-aligned reset wakes, then settles chassis and unsprung velocities together after a sustained quiet-input window without changing normal driving suspension rates. A deterministic exact-car/exact-track unit regression reproduces the prior five-second 0.050 rad/s body rocking and 0.243 m/s wheel motion at the 110 m incline before verifying no late body rocking, wheel oscillation, or contact chatter.',
    '2026-08-11 15:34 EDT - Stopped WRX2 suspension motion from persisting after hard impacts and route-center resets without changing its springs, dampers, anti-roll rates, travel, or tire grip: reset hold now snapshots and restores all four equilibrium suspension/contact states instead of advancing hidden wheel oscillators, temporary support changes cannot release a parked reset, low-energy near-stationary heave/pitch/roll and unsprung motion settle atomically, explicit attitude damping is applied in body rather than world axes, and the exact Studio Sprint 2 first-jump regression now injects stale crash impulses before proving three seconds of motionless post-impact reset.',
    '2026-08-10 19:56 EDT - Removed the remaining late-race Dynamic Track State checkpoint spike while preserving snapshot version 1 and exact legacy checksum bytes: automatic checkpoints now pre-copy cells with mutation invalidation, freeze only at deterministic advance boundaries, capture later mutations through copy-on-write, assemble canonical cell arrays in bounded slices, and feed the existing FNV checksum through a resumable canonical JSON stream instead of cloning, canonicalizing, stringifying, and hashing the entire state in one frame. The 8,192-event limit is now a soft cap until a checkpoint commits, runtime diagnostics expose checkpoint phase/backlog/hash progress/overage/slice cost, weather transitions persistently wake every historical cell across bounded steps, and weather history prunes only through the oldest captured cell update. Exact hashing, soft-cap completion, weather wake, replay, sync, render-partition, hitch, restore, and one-hour 20-car regressions cover the new path.',
    '2026-08-10 18:30 EDT - Removed the recurring Dynamic Track State weather hitch without changing its 100 ms numerical integration or snapshot/checksum format: dry tire contacts no longer create three empty neighbor cells, deterministic environmental-active queues retire settled cells instead of rescanning every historical sample, conservative water flow reuses persistent sorted source/delta/key buffers, and bounded per-frame maintenance slices rebuild derived activity outside physics commits. Runtime diagnostics now expose total versus environmental-active cells, receiver allocations, flow-buffer high-water, and prepared-cell counts; focused determinism, render-partition, restore, sparse-world, and allocation regressions cover the change.',
    '2026-08-10 17:30 EDT - Fixed the worker-era body/wheel disconnect regression and gated automatic migration until browser crash parity is proven: normal races again keep render-thread vehicle authority by default, while explicit forced-worker testing now uses a versioned coherent presentation snapshot with body pose and motion, authoritative wheel hubs, suspension/contact geometry, loads, temperatures, gear, and RPM. Rolled and inverted wheels no longer derive their centers from contact point plus world-up radius; rendering, race compatibility consumers, AI, and Physics Debug select the same worker presentation state without mutating or reading the dormant runner. Focused rollover, bridge, protocol, migration, and authority regressions cover the corrected handoff.',
    '2026-08-10 17:03 EDT - Fixed the passive post-jump energy leak and moved normal race authority off the render thread without lowering fidelity: body manifolds now use accumulated projected normal and friction constraints so restitution is not reapplied on every solver iteration, suspension bump and droop stops discard velocity directed farther into the hard limit, and the exact zero-input Studio Sprint2 / WRX2 landing drops from eight repeated impacts to two with every impact losing energy. Module-worker authority now defaults to automatic, pre-copies the packed race surface during countdown, uses a drift-corrected 120 Hz clock, falls back cleanly if initialization fails, and caches expensive Physics Performance summaries. The current Android 60 FPS capture measures 6.90 ms fixed-step p95, about 312 steps/s, and zero backlog; 67 focused contact, landing, worker, protocol, and accounting tests pass.',
    '2026-08-10 15:43 EDT - Fixed Select-hold race resets so the authoritative car remains parked at its validated route-center pose until meaningful forward or reverse drive input arrives: the deterministic runner now holds chassis and wheel motion while continuing contact evaluation, releases for drive, collision, recovery, or lost support, preserves the state through snapshots and replay, and carries the flag through the compact worker reset command. Focused sync-reset, worker authority, and protocol regressions cover idle holding, steering/brake/handbrake non-release, reverse wake-up, snapshot restoration, and transfer-buffer parity.',
    '2026-08-09 20:20 EDT - Made designated VehicleDynamicsRunner worker qualification fail closed: the performance runner now requires a stable reference-machine ID when marking a capture designated and offers an enforcement mode that still writes diagnostics but exits unsuccessfully unless measured fixed-step p95, throughput, and backlog all pass. Portable captures and incomplete designation attempts are verified rejection paths; the 44-test focused suite remains green, while activation still awaits the external controlled-machine artifact.',
    '2026-08-09 20:18 EDT - Reduced qualification observer effect without weakening the physics workload: fixed-step cost records now retain exact elapsed time and backlog while full subsystem timings and counters remain on frame records, avoiding duplicate per-step diagnostic graphs. The exact Studio Sprint2 checksum and zero backlog remain unchanged; the latest full local Android run measured 245.49 steps/s and 8.429 ms p95 against the 8.333 ms budget, so the worker gate remains closed. The focused worker and performance suite passes 44 tests.',
    '2026-08-09 20:16 EDT - Hardened the staged VehicleDynamicsRunner worker qualification evidence: designated reports must contain measured fixed-step p95 samples and can no longer qualify from the legacy render-frame-divided estimate; performance regressions now validate the realtime 120 Hz and high-fidelity 360 Hz captures as separate deterministic contracts instead of making false before/after reduction claims across incomparable workloads. The combined worker, parity, collision, metrics, and performance suite passes 43 focused tests; no new designated-reference-machine artifact is present, so activation remains gated.',
    '2026-08-09 15:10 EDT - Closed mutable-environment parity for the staged VehicleDynamicsRunner worker: a 96-byte transferable update carries live weather forcing plus player aero, powertrain, brake, and tire damage without terrain or telemetry; the worker evaluates deterministic gusts at its own fixed-step time, applies weather to worker-owned Track State, derives live wake sources from the previous authoritative states of player and active AI, isolates each AI environment while sharing immutable packed terrain, and advances the render event sequence for authoritative collision impacts. An exact 120-step regression now matches complete runner state across a mid-run storm and damage transition, and the focused worker/ownership suite passes 39 tests.',
    '2026-08-09 15:02 EDT - Staged the dedicated VehicleDynamicsRunner worker migration behind a mandatory designated-host single-thread qualification gate: the module worker owns its 120 Hz clock, player and dynamically active AI runners, live wake sources, collision world, replay input timelines, tire and suspension state, and deterministic Track State mutations; handoff waits until live Track State exists; one-time packed terrain and static-collider definitions transfer into worker-owned indexes; 64-byte control buffers and 184-byte render snapshots avoid telemetry and geometry churn; presentation interpolates only the newest two snapshots; wheel spin advances at authoritative fixed-step boundaries; recycled buffers bound allocation; and cached live Physics Performance reports independent worker/render p50/p95/p99 plus backlog without sorting full sample windows every render frame. The 80-collider worker stress run explicitly sustains at least 120 authoritative steps per real second, drains scheduler backlog to zero, and keeps the render heartbeat responsive. The acceptance gate consumes actual fixed-step samples; dry Track State flow skips needless graph construction and nested cost timers reuse allocation-free tokens with exact checksums preserved. Post-optimization local Android captures reached 8.37 ms p95 and 253.99 steps/s, but a thermally noisy repeat measured 13.18 ms, so worker activation remains intentionally disabled pending a controlled passing designated-reference-machine report.',
    '2026-08-07 08:51 EDT - Replaced render-frame road-edge correction with authoritative fixed-step static-collider CCD: a cached deterministic broadphase now prepares planes, oriented boxes, convex hulls, and triangle meshes for road walls, barriers, buildings, scenery, and test obstacles; the compound vehicle sweeps translation and rotation to the earliest TOI, expands simultaneous face contacts into an ordered manifold, applies friction and restitution at physical points, uses position-only split correction, and resweeps the remaining substep without queuing or resetting; solid scenery damage now follows the same fixed-step contact history; Physics Performance exposes static candidates, narrowphase tests, contacts, and CCD cost; and the complete exact-134.112-m/s suite passes all 30 floor/wall render-rate, 120/240/360 Hz, and 250 ms hitch matrices with zero tunnelling or gameplay recovery.',
    '2026-08-06 16:03 EDT - Added full-runtime WRX2 high-energy collision regressions without changing collision behavior: exact 300 mph (134.112 m/s) plus 30, 60, and 100 m/s impacts now exercise the real VehicleDynamicsRunner, ContactPatchTireModel, compound body, wheels, suspension, prepared-surface triangles, and normal Race surface-query path across six floor geometries, six wall approaches, four wall thicknesses, five render rates, 120/240/360 Hz contact rates, and a 250 ms pre-impact render hitch; assertions cover swept TOI, penetration, complete obstacle crossing, finite state and damage, catastrophic recovery, route resets, energy, and replay checksums. All 24 floor catalog cases and all 24 high-energy floor matrix checks pass; the deliberately red baseline exposes 72 of 96 canonical wall cases plus the 45-degree and spinning 0.05 m wall matrices as current deep-penetration/catastrophic-recovery failures, preserving the defects for the subsequent collision fix.',
    '2026-08-06 14:39 EDT - Finished the production terrain-query optimization audit without reducing physics fidelity: adaptive body-support entries now return to reusable pools instead of being deleted and recreated, prepared body contacts are built in allocation-light scalar loops, lower-hull CCD checks exact local triangle continuity before activating the full compound envelope, and wheel-cylinder height/sweep work runs only for actual discontinuities, impending support loss, or full-bump geometry; granular counters now separate support-entry, batch-target, point-buffer, and buffer-growth allocations; and the regenerated exact Studio Sprint2 / WRX2 acceptance report preserves all five locked checksums with zero backlog, out-of-frame queries, or recovery while reducing full geometry calls by 98.9-99.0%, prepared-triangle visits by 91.5-92.3%, and temporary objects by 85.5-87.7% versus the committed baseline.',
    '2026-08-06 13:51 EDT - Replaced repeated authoritative terrain work with one revision-aware PhysicsTerrainQueryFrame per 360 Hz vehicle-contact substep: wheel centers, true packed-batch footprints, iterative tread projection, body broadphase/support/CCD, wheel-cylinder activation/sweep, and penetration validation now share revision-cached local triangle indices, copy-free triangle views, typed query/result buffers, exact clipped height bounds, material/projection caches, and deterministic statistics; body collision now selects swept-AABB, lower-hull, or full adaptive compound support by clearance, attitude, and terrain continuity while wheel-cylinder CCD activates only for real discontinuity/leading-contact risks; the exact twelve-second Studio Sprint2 / WRX2 acceptance capture preserves all five locked physics checksums with zero backlog, out-of-frame queries, or recovery recalculations while reducing full geometry queries by at least 98.8%, fully accounted prepared-triangle visits by 90.6%, and practical temporary objects by 85.1% versus the committed baseline; and the report explicitly separates portable operation budgets from the designated-host 8 ms p95 measurement.',
    '2026-08-06 08:14 EDT - Separated ordinary authoritative contact stabilization from visible gameplay recovery without retuning tires, suspension, geometry, or physics rates: residual body overlap through 30 mm now remains manifold and split-position settling with only inward normal velocity removed; moderate overlap uses the current or immediately previous non-penetrating local CCD frame, preserves route distance and terrain-tangent motion, and immediately rebuilds tread, suspension, body, and sidewall contacts without entering recovery history; historical or route recovery now requires body/CG/lower-support submersion, a non-finite state, or at least 250 ms of unavailable authoritative terrain; transient terrain gaps retain the last valid local collision normal and suppress unsupported tire force; telemetry separately counts ordinary corrections, local rollbacks, catastrophic historical recovery, catastrophic route recovery, and gameplay resets; and regressions cover ten-second scrapes, persistent overlaps, bottom-out, transient terrain gaps, 100 mm TOI rollback, 400 mm submersion, non-finite state, and a thirty-second Studio Sprint 2 hill replay.',
    '2026-08-05 23:10 EDT - Added opt-in production physics cost accounting without changing solver inputs or outcomes: nested inclusive/exclusive timers now cover Race authority, environment, wheel and footprint queries, iterative tire contact, route and baked sampling, body broadphase/support/sweep/manifold, wheel-cylinder activation/triangle/height sweeps, penetration, recovery rebuilds, Track State, and telemetry; per-frame and per-step counters expose geometry, projection, triangle, support, CCD, recovery, backlog, and practical temporary-object volume; the Race Debug menu now includes a Physics Performance HUD with FPS, p50/p95/p99, backlog, top-five subsystems, queries, CCD rates, and recovery state; and a committed 56,448-triangle Studio Sprint 2 / WRX2 baseline runs flat road, hill, crest, loose terrain, jump, and scrape sections for twelve seconds at 30, 60, 90, 120, and 144 FPS with incident and surface debugging disabled. The baseline identifies wheel-cylinder height sweep as the largest inclusive cost, followed by penetration/body sweep work and roughly 1.5-2.0 million route/baked queries per run, with zero recoveries.',
    '2026-08-05 20:32 EDT - Added deterministic finite-width wheel-cylinder sweep collision at the authoritative 360 Hz contact boundary: every wheel now sweeps leading tread and both sidewall faces from its previous to proposed hub pose against the same prepared terrain triangles and explicit surface samples used by chassis collision; curb, step, sharp-ramp, diagonal, partial-width, and narrow-crest impacts resolve at their physical time and point through normal and friction impulses without powered sidewall traction, artificial climbing force, or body split-impulse lift; a local tangent-plane broadphase keeps ordinary smooth grades on the tire and suspension path instead of invoking the expensive collision manifold continuously; and WRX2 curb outcomes match at 5, 15, and 30 m/s across 30, 60, 90, 120, and 144 FPS.',
    '2026-08-05 18:31 EDT - Demoted penetration recovery to a non-recursive catastrophic fallback after ordinary full-bump, manifold, rollback, iterative-constraint, and split-impulse contact resolution: shallow residuals now require stalled progress across multiple complete 120 Hz chassis steps; stable incident IDs combine quantized route/world regions, prepared triangles, body features, terrain sources, and body profile; every historical or route source remains blacklisted until the car has stayed clear and travelled away; one historical retry escalates once to a verified terrain-aligned backward route search and then to a stopped hard failure; recovery removes velocity into the blocking normal, caps safe tangent speed, clears pitch/roll rate, and never invents route distance zero; and the exact Studio Sprint 2 hill now runs ten deterministic seconds at every render rate without invoking catastrophic recovery.',
    '2026-08-05 16:48 EDT - Made rising terrain physically load-bearing through full suspension bump instead of deleting wheel support: authoritative SurfaceSample records now preserve valid height, normal, region, source, triangle, query position, and invalid reasons without coercing missing terrain to sea level; wheel contact explicitly separates no terrain, droop loss, airborne proximity, ordinary support, full-bump support, sidewall geometry, wrong-side support, and body occlusion; upper overtravel clamps hub travel while tire compliance, progressive bump stops, and hard stops apply force and moment at the tread; three-pass prepared-surface projection converges wheel height and normal; and the underbody split constraint now rotates as well as translates without adding rebound energy, allowing the exact Studio Sprint 2 WRX2 hill capture to clear at 10-60 mph, every render rate, and 120/240/360 Hz without emergency recovery.',
    '2026-08-05 15:40 EDT - Replaced the flat, empty-tire Studio Sprint 2 recovery placeholder with an automatically captured five-second authoritative physics incident: a deterministic fixed-step recorder now retains two seconds of control, chassis, wheel, suspension, tread-validity, body-contact, exact tire/body surface-query, prepared-triangle, route, and recovery history; exports compact checksummed JSON with the saved race and WRX2 identities plus the exact swept prepared-world triangles; and a full ContactPatchTireModel replay at 30, 60, 90, 120, and 144 FPS reproduces the current 275-279 mm front compression request against 150 mm travel, four-wheel reach loss, body-below-terrain recovery, and repeated recovery region. The no-failure acceptance remains explicitly marked expected-failing for the subsequent physics-fix milestone.',
    '2026-08-05 10:17 EDT - Fixed the Studio Sprint 2 body-dip, distant-reset, and hitch-driven frame-collapse regressions without retuning the WRX2: body collision now rolls every ordinary contact back to the latest non-penetrating swept pose before publishing it; emergency recovery selects only recent, reachable, verified poses from a bounded deterministic path history and otherwise projects the failed world position onto the nearby route instead of falling back to race start; Race caps each render frame to eight authoritative catch-up steps while preserving the remaining deterministic backlog; tire centers share the footprint terrain batch; adaptive compound refinement is explicitly bounded; and Physics Surface debug streams a camera-local terrain neighborhood instead of rendering all 56,448 triangles twice.',
    '2026-08-05 00:28 EDT - Finished atomic authoritative collision recovery and the Studio Sprint 2 terrain regression boundary: emergency depenetration now discards the failed manifold, suspension correction, body contacts, and sidewall contacts before restoring and immediately rebuilding orientation, velocity semantics, suspension geometry, wheel kinematics, tread validity, and clean compound contacts; repeated safe-state failures select older verified path states and escalate directly to local route recovery when no safe rewind exists; custom convex bodies now contribute interpolated edge and face support, prepared varying terrain tiles adaptively refine compound support without increasing flat-terrain cost, and tread support revalidates the complete suspension-mount-to-contact path after final hub compression; a source-hashed serialized Studio Sprint 2 fixture now locks recovery neighborhoods, baked-versus-analytical heights, penetration tolerance, and exact 30/60/90/120/144 FPS outcomes.',
    '2026-08-04 17:34 EDT - Calibrated the complete WRX2 controller-to-rack path for dirt response: removed the compounded gamepad target-plus-wheel filtering in favor of one data-driven response stage with visible first-frame motion, approximately 100 ms half response, and approximately 330 ms ninety-percent response at 30 mph; retained progressive digital steering and direct simulation-wheel input; carried intent, target, filtered output, rack limits, independent wheel angles, front loads, grip, utilization, slip, lateral force, aquaplaning support, yaw, body slip, turn radius, and throttle friction use through replayable fixed-step telemetry with explicit failure classification; and changed the browser feel fixture to drive the real gamepad-axis path instead of assigning both internal steering states.',
    '2026-08-04 16:44 EDT - Replaced the generic sedan probe cage with saved, replayable compound Physics Body profiles for car, SUV, pickup, and custom vehicles: authoritative terrain collision now samples solid lower-body, hood, cabin, trunk, cab, bed, or editor-authored pieces through dense deterministic face/edge/corner support features and a conservative broadphase; wheel cylinders submit non-powered sidewall collision separately from tread propulsion and physical suspension reach; WRX2 carries explicit production body dimensions while retaining its calibrated CG and inertia; legacy road cars migrate safely by vehicle class; Car Editor exposes preset, dimensions, clearance, overhangs, profile measurements, CG, friction, restitution, reset, and custom-box controls; and Race Physics Surface draws the compound pieces, wheel hubs, suspension axes, tread points, body/sidewall contacts, normals, penetration, broadphase, and invalid tread-contact reasons.',
    '2026-08-04 14:40 EDT - Hardened authoritative terrain contact against submersion and wrong-side tire support: every wheel now separates geometric proximity from valid tread support using physical suspension reach, support-axis alignment, tread-normal compatibility, and body occlusion, so inverted or side-resting cars can spin unsupported wheels without tire propulsion; chassis body probes sweep translation and rotation to the earliest terrain crossing before resolving the remaining 360 Hz substep; strict post-solve penetration checks retain a replayable last non-penetrating pose and deterministically recover stalled, deeply submerged, invalid-envelope, or fully buried states with bounded velocity and explicit telemetry; and fixed-step diagnostics compare baked and physics terrain heights beneath CG, hull, and wheel samples with discrepancies over two centimeters flagged without affecting simulation state.',
    '2026-08-04 08:06 EDT - Validated physical jump takeoff and airborne attitude without auto-leveling: the authoritative 360 Hz runner now records front/rear wheel release times, each axle\'s preceding suspension impulse, underbody crest contacts, final-contact pitch rate, full in-flight pitch history, and landing quaternion; wheel angular acceleration applies an equal-and-opposite inertia-derived impulse to the chassis for airborne throttle and braking while steering contributes no unsupported yaw moment; and replayable Studio Sprint 2 crest fixtures at 40, 60, and 80 mph remain exact across 30, 60, 90, 120, and 144 FPS while clearing the underbody and preserving torque-free attitude.',
    '2026-08-04 07:30 EDT - Unified authoritative impact integration at the 360 Hz contact boundary: every tire, suspension, aerodynamic, gravity, and angular impulse now advances one temporary chassis state before body contacts solve against that same updated pose; body penetration uses bounded split positional correction with no rebound velocity, while physical closing speed alone drives restitution and friction; tire and suspension support are no longer double-applied at the 120 Hz public boundary; and deterministic impact telemetry records kinetic energy, per-wheel suspension and vertical-tire impulse, body normal/friction/restitution contributions, zero penetration-bias impulse, positional correction, and successive rebound apexes across flat, pitched, one-wheel, nose, side, and Studio Sprint 2 jump fixtures.',
    '2026-08-03 19:50 EDT - Corrected WRX2 loose-surface steering and authoritative handbrake control: Sport and Accessible retain speed-, contact-, and aquaplaning-safe rack limits without deleting steering from current grip or friction-circle utilization, fixed-step telemetry exposes requested/permitted/actual rack and front contact state, replayable double-tap R pulses now use one deterministic 360 ms command for physics and visuals, and handbrake operation cuts combustion torque, opens AWD center coupling, suppresses TC/ESC through rear-slip recovery, and leaves rear mechanical lock outside ABS without adding yaw impulses.',
    '2026-08-03 18:09 EDT - Added active collidable doodads to the Race Physics Surface debug view as translucent terrain-conformed collision cylinders built from each doodad\'s authoritative hitbox width and height; removed and flattened doodads are excluded so the overlay shows the collision volumes the car can currently hit without changing simulation state.',
    '2026-08-03 16:11 EDT - Completed final authoritative vehicle-interaction hardening: road-edge impacts now submit deterministic contact geometry to VehicleDynamicsRunner, which resolves authoritative world-velocity impulses, friction, penetration correction, application-point moments, and replay history without external pose mutation; Sport and Accessible rack limits now come from supported front contact loads, local friction, tire utilization, aquaplaning, speed, and contact count while Simulation remains direct; yaw stability is one capacity-aware understeer, oversteer, and countersteer ESC brake controller with no competing body-yaw moment; physical sleep requires balanced force and moment plus sufficient static friction and cannot hold a steep low-grip slope; automatic forward shifts use fixed-step RPM, driven-wheel speed, pedals, projected ratios, and hysteresis; and physics damage variation is deterministic by race, vehicle, event, and sequence.',
    '2026-08-03 14:16 EDT - Removed remaining vehicle-authority leaks: legacy brakeBalance now maps exactly into the unified per-wheel brake pipeline, persistent wheel-specific brake damage and stability demand are applied before ABS and grip limiting, disabling stability suppresses every selective brake and body-moment intervention, normal tire wear is owned only by fixed-step accumulated tire work, route progress follows authoritative world velocity along the road during large slides, controller return uses front contact-patch aligning torque and physical trail while simulation wheels receive no hidden correction, safety and collision paths use explicit ground/body speed semantics, and pitch/yaw/roll rate fallbacks map to the correct world axes.',
    '2026-08-03 08:43 EDT - Completed authoritative wheel-torque and tire-energy integration: the fixed-step runner now owns RPM-dependent engine torque, clutch and converter coupling, interrupted shifts, rev limiting, engine braking, drivetrain losses and damage, front/rear-biased per-wheel brakes, ABS, traction, launch, and stability interventions; exposes ground, body-longitudinal, body-lateral, and signed travel speeds; uses total ground speed for controller steering safety; carries continuous C1 brush-to-sliding tire forces with relaxation and breakaway recovery state; accumulates every tire substep\'s friction, flex, load, conduction, and water-cooling work into temperature, pressure, wear, and Track State mutation; and applies roll assistance with roll-axis inertia.',
    '2026-08-03 07:44 EDT - Fixed the shared virtual gamepad Select control in Race, Level, and Car playtests: touch-down now enters the same frame-partition-independent one-second hold path as a physical controller, matching-pointer release preserves short-tap camera switching, completed holds reset without switching views, and countdown, pause, touch-cancel, and playtest cleanup clear the held state safely.',
    '2026-08-02 21:17 EDT - Stabilized authoritative driven-wheel inertia at the static-contact boundary: sub-limit launch torque now resolves the tire\'s elastic equilibrium slip instead of allowing the explicit 360 Hz reaction torque to overshoot repeatedly through rolling speed, eliminating false stock-car wheelspin without suppressing real over-capacity burnouts, locked wheels, or post-peak slip.',
    '2026-08-02 16:04 EDT - Added gamepad Select hold recovery across Race, Level, and Car playtests: holding Select for one second after GO shows bounded HUD progress, captures current race progress, fades to an upright authoritative route-center reset with chassis motion cleared, and suppresses camera switching; short Select presses still change camera on release while automatic edge recovery retains its momentum-preserving behavior.',
    '2026-08-02 12:30 EDT - Stabilized the authoritative WRX2 over Studio Sprint 2 terrain without retuning its physical profile: suspension damping now uses signed sprung-to-unsprung motion and a continuous digressive high-speed slope instead of multiplying sharp-bump forces into the six-times-load cap, first contact initializes hub travel geometrically, gravel is selected once by the tire compound while Track State contributes only local and snow-condition scaling for player and AI, and Physics Surface debug now reports composite conditions such as GRAVEL + SNOW with bounded traction ticks, wrapped condition keys, and compact per-wheel readouts.',
    '2026-08-02 10:36 EDT - Corrected the mid-race WRX2 traction and stability regressions on Studio Sprint 2: geometry-batched tire footprints once again provide full multi-sample support instead of silently falling back to one point, saved WRX2/SPT playtests rehydrate the calibrated inertia, suspension, anti-roll, tire, steering, and aero profile, Track State conditions now enter tire friction once with continuous wet/snow/ice compound blending instead of a 2 mm snow grip cliff, and Physics Surface debug colors authoritative surface traction while labeling each wheel\'s actual grip, load, support, steering angle, and aquaplaning state.',
    '2026-08-02 08:30 EDT - Restored single-car authoritative physics throughput without lowering the WRX2 120 Hz chassis or 360 Hz tire/contact rates: wheel footprints now use exact geometry-only batches instead of repeating Track State and material work, body contacts use a conservative prepared-surface elevation broadphase before the unchanged manifold solver, route projection uses an exact bounded spatial index, hard-corridor contact samples skip unused raw-terrain normals, and the Track State overlay no longer builds an unused atlas every frame.',
    '2026-08-02 02:47 EDT - Recovered race physics frame time after the authoritative vehicle rollout: the player keeps 120 Hz chassis and 360 Hz contact fidelity while AI uses the same force laws at 120 Hz; the runner now avoids JSON deep-copy churn inside contact substeps, supports transient/latest/disabled telemetry retention without changing replay-safe history mode, removes duplicate AI contact queries, reuses one wake snapshot per frame, and exposes non-checksummed operation counters for performance regression coverage.',
    '2026-08-01 13:03 EDT - Replaced centered speed-squared vehicle drag with fixed-step physical aerodynamics: relative airflow now combines chassis velocity, authored atmospheric wind, deterministic gusts, crosswind, yaw, and elevation/temperature air density; data-driven maps resolve drag, front/rear downforce, center of pressure, ride height, rake, ground effect, floor stall, diffuser response, active aero, extreme-yaw lift, and panel-damage imbalance at physical application points; and bounded deterministic wakes from every active player/AI car now combine drafting, dirty-air downforce loss, turbulence, crosswind movement, distance decay, and overlaps while AI telemetry identifies slipstream, braking, cornering, overtaking, and crosswind risk.',
    '2026-08-01 12:24 EDT - Added physical suspension geometry and unsprung contact dynamics to the authoritative 360 Hz wheel loop: parameterized MacPherson, double-wishbone, multilink, trailing-arm, and solid-axle definitions now resolve camber gain, toe and bump steer, caster, kingpin inclination, scrub radius, trail, roll center, motion ratio, anti-dive, anti-squat, and wheel rate; wheel/hub mass, tire vertical compliance, progressive springs, velocity-sensitive damping, top-out, and bump stops persist per wheel; and bounded four-to-eight-sample tire footprints provide partial curb support, banked normals, seam stability, and large-gap rejection for player and visible AI cars.',
    '2026-08-01 09:11 EDT - Coupled authoritative tire contacts to Dynamic Track State: each wheel now advances independent tread, carcass, and internal-air temperatures with friction, flex, load, surface conduction, airflow, water, snow, and ice heat transfer plus temperature-driven pressure; standing water now creates tire-, load-, speed-, slip-, tread-, pressure-, and patch-dependent evacuation and hydrodynamic lift before longitudinal, lateral, and aligning forces; the same contact work drives water displacement, drying lines, rubber, loose-material sweep, and carry; and a bounded streamed visual atlas/layer exposes dampness, puddles, rubber, marbles, dirt, mud, oil, debris, snow, and ice without entering deterministic simulation state.',
    '2026-07-31 15:14 EDT - Corrected authoritative WRX GT/SPT body motion and asphalt power delivery: VehicleDynamicsRunner now consumes the saved 58/42 weight balance and independent front/rear spring, bump, rebound, travel, anti-roll, and track geometry; full-quaternion wheel attachments make physical suspension forces restore pitch and roll without soft hidden chassis springs; selected gear, final drive, engine torque, efficiency, and redline now bound driven-wheel energy so tires cannot remain artificially saturated or accelerate the car after throttle release; clutch-disengaged countdown revs cannot create tire torque; stationary engine braking no longer invents motion; and the Three.js car preserves the runner pitch convention so acceleration and hills raise rather than dip the nose.',
    '2026-07-31 15:04 EDT - Applied the selected global Color, Sepia, or Night Vision display mode to live Race and Car playtest viewports, including their HUD and pause surface, while leaving editor chrome, handheld controls, and passive Car previews unfiltered.',
    '2026-07-31 13:23 EDT - Fixed authoritative cars launching or floating after hills and respawns: known terrain geometry now unloads suspension and tire forces beyond physical droop instead of restoring fake static load, airborne steering/braking assists are contact-scaled to zero with visible suppression telemetry, hard landings use a bounded non-energizing suspension constraint, formation velocity no longer cancels gravity, and route-center reset places the fixed-step chassis at sampled surface height plus CG while clearing stale motion and impulses.',
    '2026-07-31 13:16 EDT - Made the simulated Race and Car playtest G/R buttons act as Select/Back while paused, including submenu toggles and navigation, without leaking throttle, brake, handbrake, or held-pointer state into the race.',
    '2026-07-31 12:47 EDT - Made VehicleDynamicsRunner the shared authoritative Race, Level, Car, and visible-AI chassis: one 120 Hz rigid body now owns 3D pose, velocity, wheel, suspension, tire, and powertrain compatibility state while four independent tire patches, fixed-substep terrain and Track State contacts, suspension forces and application-point moments, gravity, aero, engine braking, explicit HandlingAssist interventions, and collision impulses produce the trajectory without the legacy planar follower. Corrected yaw-oriented initial velocity and steering coordinates, added telemetry-visible Sport handbrake rotation, made visible AI resample each wheel inside tire substeps, and bounded live telemetry and control histories independently from replay records.',
    '2026-07-31 08:22 EDT - Replaced the Vehicle Dynamics shadow force clamp with an independent four-contact-patch brush tire model featuring Ackermann steering, signed longitudinal and lateral slip, wheel angular dynamics, combined-slip saturation and post-peak sliding, pneumatic trail and aligning torque, banked-surface world forces with physical application points, and existing compound, pressure, size, temperature, wear, damage, load-sensitivity, and local Track State inputs.',
    '2026-07-31 01:10 EDT - Added the deterministic 120 Hz Vehicle Dynamics shadow runner with independently configurable 360 Hz tire/contact substeps, timestamped interpolated controls, replayable snapshots and telemetry, bounded hitch recovery diagnostics, legacy-state difference recording, and twelve render-partition-invariant validation fixtures without gameplay write-back.',
    ...
    '2026-07-25 15:16 EDT - Replaced the Car Editor Art direct-to-Body jump with shared Exterior, Interior, and Camera Settings categories; added independent dashboard and rotating steering-wheel overrides with procedural fallbacks, plus a saved per-car Fixed Rear chase-camera mode with temporary manual look.',
    '2026-07-25 14:50 EDT - Added lower-right Reset Size controls for Car Editor body, tires, brakes, and shadow, restoring each active layer to 1.00x scale and zero position while preserving its selected artwork and visibility.',
    '2026-07-25 14:32 EDT - Made overridden car bodies and their authored shadow, add-on, brake, and tire-offset layers scale from live physical-width camera projection, keeping them synchronized with tire billboards during terrain-safe chase-camera dolly.',
    '2026-07-25 12:29 EDT - Routed the simulated Race and Car playtest D-pad through pause-menu navigation and setting adjustment while paused, with touch-direction edge tracking that prevents held-pointer jitter from skipping rows.',
    '2026-07-25 12:11 EDT - Unified Level, Race, and Car playtest pause presentation through one scaled in-game text-menu renderer, preserving Race/Car Settings and Debug while removing the blue editor-button chrome that was painted over their pause rows.',
    '2026-07-25 12:07 EDT - Anchored overridden tire graphics to absolute projected physics wheel centers, gave each tire billboard physical-radius perspective sizing and depth ordering, and retained saved tire offsets as a single final adjustment.',
    '2026-07-25 11:51 EDT - Moved the shared third-person race and Car Editor camera seven meters farther back to a ten-meter total chase distance, while keeping it three meters above the live physics body.',
    '2026-07-25 11:27 EDT - Moved the shared third-person race and Car Editor camera to an absolute three meters behind and three meters above the live physics body, while retaining terrain clearance and the existing chase-direction smoothing.',
    '2026-07-25 08:32 EDT - Aligned the Car Editor live Geom overlay and authored body, tire, and shadow graphics with the active Three camera projection of the physics body, wheel centers, and ground contact, and removed the duplicate editor-only Geom pass.',
    '2026-07-25 08:14 EDT - Replaced race terrain stitching with a canonical road-origin indexed mesh: the road, apron, shoulder, and outward-grown terrain now share exact boundary vertices, rendering and physics consume the same upward-wound triangles, and the former seam underlap geometry is no longer emitted.',
    '2026-07-25 07:38 EDT - Aligned the normal third-person player car with the Physics Surface reference by restoring the dynamic Three car at the authoritative live world position and rigid-body height, while preserving authored body, tire, and shadow layer overrides.',
    '2026-07-24 23:47 EDT - Fixed Car Editor freezing on entry by removing an out-of-scope terrain-stat variable reference and skipping editor-only full-mesh diagnostics while its embedded Studio Sprint preview builds the same rendered and physical race world.',
    '2026-07-24 23:21 EDT - Added a Race Editor and pause-menu Physics Surface debug view that renders the exact collision sampler triangles and live wheel contacts, and moved large painted terrain maps to a road-aligned transition join so bend seams no longer inherit saw-tooth height-grid slivers.',
    '2026-07-24 20:31 EDT - Fixed the post-jump throttle and brake lockout by replacing the permanent one-frame rollover latch with sustained support-aware detection, automatic upright recovery, reset cleanup, and consistent forward/reverse control gating.',
    '2026-07-24 19:38 EDT - Restored responsive race steering after the contact and tire-physics changes by retuning analog/digital input buildup, using the full control range proportionally, and removing duplicate powered-corner steering penalties while preserving real friction-circle and front-contact limits.',
    '2026-07-24 18:56 EDT - Fixed random race power loss by replacing demand-derived wheelspin with persistent measured driven-wheel rotation and slip, making traction control respond to actual contact slip, stabilizing contact loads, and preventing unloaded or backward automatic upshifts.',
    '2026-07-24 17:47 EDT - Removed physical terrain spikes at road seams by welding nearby clipped boundary vertices, discarding boundary-only sliver faces, stabilizing bend-side transition ownership, and slope-limiting only the transition mesh while keeping the rendered and collision surfaces identical.',
    '2026-07-24 17:15 EDT - Removed needle-like terrain seam triangles by turning the clipped seam edges into continuous chains and collapsing only sub-five-centimeter fragments before building the buried underlap.',
    '2026-07-24 16:55 EDT - Removed the remaining jagged terrain seam spike by revalidating inherited clipping vertices against the canonical road boundary instead of trusting stale exact-seam elevations.',
    '2026-07-24 16:22 EDT - Closed the terrain-side underlap misses by deriving its buried geometry from every actual clipped road-facing terrain edge instead of approximating the terrain boundary from road sections.',
    '2026-07-24 15:59 EDT - Replaced the tiny one-sided apron seam skirt with opposing one-meter underlaps that sink three meters beneath the road and terrain, creating a buried criss-cross join while leaving the visible driving surface and collision bake unchanged.',
    '2026-07-24 15:41 EDT - Closed the final apron-to-terrain hairline with a shallow render-only seam skirt, and removed recurring playtest stalls by caching the physical top-speed solve, retaining ghost sample storage, and reusing the full static Three road/overlay world.',
    '2026-07-24 14:47 EDT - Replaced the flawed adaptive terrain subtraction with a uniform ten-meter conforming mesh, densely sampled visual road extensions, true polygon-intersection gating, strict seam validation, and shared-edge triangulation; the saved Studio Sprint bake now has zero internal openings, non-manifold edges, seam mismatches, or magenta diagnostics.',
    '2026-07-24 11:35 EDT - Replaced stretched corridor guard sheets with the painted five-meter heightmap grid on terrain-rich races, welded that grid to the canonical road apron transition, and routed runtime ground height and normals through the exact unlifted rendered triangles.',
    '2026-07-24 10:59 EDT - Corrected the hidden Studio Sprint gravel-hill collision profile so terrain support cannot create an oscillating, steeper-than-visible roadbed, and made tire friction heat surface-aware while preserving contact gating for player and AI cars.',
    '2026-07-24 09:18 EDT - Fixed the Studio Sprint gravel-hill stall by preserving usable torque when an AWD axle or FWD/RWD drive wheel partially unloads, while separating airborne free-spin from contact slip so unloaded tires do not generate friction heat, wear, sound, or particles.',
    '2026-07-24 02:26 EDT - Fixed the local Playwright static server project-file endpoint to decode compact gzip-backed documents before returning them, so Studio Sprint opens as its saved race document instead of an empty compression envelope.',
    '2026-07-24 02:08 EDT - Added road grade and grade-force readouts to the race debug HUD so manual hill-climb and downhill-brake validation can verify the corrected road-angle gravity model while driving instead of relying only on unit-test margins.',
    '2026-07-24 02:00 EDT - Tightened another render-position mismatch from the physics audit: projected AI/other-car sprites now carry their actual world pose into custom art shadows and geometric debug overlays, reducing cases where rendered cars can look disconnected from their simulated position.',
    '2026-07-24 01:52 EDT - Current completion estimate is roughly 98%. We are not done because the remaining proof is still a supported browser run plus manual driving feel validation, especially third-person drift camera feel, high-power dirt/gravel throttle behavior, hill/crest transitions, and whether the telemetry matches what is seen while driving. The work is taking time because camera, tire friction, drivetrain force, grade force, wheel contact, suspension, and AI/player parity all feed back into the same feel problems, so each change needs focused regressions before the next tuning decision is trustworthy. Estimated time remaining is still about 45 to 90 minutes if the supported browser/manual pass only finds minor issues; longer if it exposes a deeper mismatch.',
    '2026-07-24 01:44 EDT - Fixed jump/landing diagnostics so landing impact is preserved before vertical velocity is settled to zero, making the physics telemetry more trustworthy when tuning crest, jump, suspension, and camera-bump behavior.',
    '2026-07-24 01:36 EDT - Added another camera-feel pass toward the remaining 3rd-person issue: the playtest camera yaw now has bounded smoothing during drift/power-spin direction changes, so it can follow travel direction without snapping the chase rig around the car in a way that reads like pivoting.',
    '2026-07-24 01:28 EDT - Current completion estimate remains roughly 97%. The hold-up is validation, not feature discovery: the latest code has focused fixes and regressions for the reported camera, rendering, drivetrain, traction, airborne, AI contact, and telemetry issues, but it still has to survive a supported browser Playwright run and a hands-on race feel pass before the physics audit should be called done.',
    '2026-07-24 01:12 EDT - Race physics audit is roughly 97% complete: third-person drift camera tracking, mid-track live world-position rendering, bounded chase-anchor lag, live 3D contact/body sync, 1200 HP dirt/gravel traction saturation, stock-versus-overpowered dirt behavior, AWD/RWD/FWD high-power regressions, tire load sensitivity, anti-roll load transfer, hill/brake/grade coverage, airborne momentum/contact, respawn height, reverse-speed behavior, automatic shifting, WRX summary/top-speed targets, AI/player bumpy-load parity, AI airborne fallback contact, and telemetry for wheel surface, terrain region, tire limits, load sensitivity, friction-circle use, contact state, and force rejection are in place.',
    'Why this is taking longer: the bugs overlap in the same systems. Camera anchoring, drawn car position, 3D wheel contact sync, terrain height, load transfer, drivetrain force limiting, tire saturation, automatic shifting, and AI/player parity all affect the same race feel, so each fix needs regression coverage before more tuning is trusted.',
    'Why this is not done yet: the remaining risk is real browser and hands-on feel validation, not a known local syntax issue. The local Android/Termux environment cannot execute Playwright browsers because playwright-core reports Unsupported platform: android, so the app-path regression is only syntax-checked locally and still needs a supported GitHub/Docker browser run. Manual checks also need another pass on camera feel, pavement boundaries, hills, reverse, and 1200 HP dirt/gravel steering with the latest AI/contact and force-loss telemetry available.',
    'Estimated remaining work: about 45 to 90 minutes if the supported browser run passes and manual playtest only finds minor tuning issues; longer if the browser gate exposes a deeper camera or traction mismatch. Next steps are to run the supported-platform Playwright spec, manually drive the updated build on http://127.0.0.1:8000, tune any remaining feel outlier, then decide whether the physics audit can be called complete.'
  ],
  recentMajorChanges: [
    '2026-07-24 20:31 EDT - Added rollover candidate/recovery timers, load-bearing support checks, control-lock telemetry, reverse contact gating, and regressions for transient landing spikes, sustained rollover, upright recovery, route reset, and full throttle/brake restoration after genuine airtime.',
    '2026-07-24 19:38 EDT - Added a proportional grip-limited steering envelope, quicker balanced input response, low-speed lateral response, contact-load authority saturation, faster tire-angle relaxation, powered-gravel steering coverage, and steering telemetry for input mode, usable angle, load authority, and remaining grip.',
    '2026-07-24 18:56 EDT - Added per-wheel angular speed and load-bearing contact state for AWD/FWD/RWD, retained usable post-peak loose-surface thrust, reset slip while airborne, exposed measured-slip and torque-cut telemetry, and covered the Studio Sprint stop-wait-restart hill plus wheel contact/free-spin behavior.',
    '2026-07-24 17:47 EDT - Made constrained terrain a single-valued surface at the road edge, removed non-surface seam curtains, smoothed skinny transition triangles against their local chords, and added validation for conflicting seam heights and near-vertical physical faces; Studio Sprint remains at zero holes, non-manifold edges, and magenta diagnostics.',
    '2026-07-24 17:15 EDT - Simplified Studio Sprint’s 896 raw terrain seam edges into 885 stable underlap spans, merging 11 micro-fragments with none omitted; the minimum emitted span increased from 2.27 mm to 6.28 cm while terrain validation stayed clean.',
    '2026-07-24 16:55 EDT - Corrected the two endpoints of Studio Sprint’s stale seam sliver, eliminated its 19 cm rise across 0.49 mm, left legitimate clipped terrain untouched, and kept validation at zero open, non-manifold, degenerate, and magenta errors.',
    '2026-07-24 16:22 EDT - Added cached terrain boundary-edge extraction with adjacent terrain materials and per-endpoint inward projection; Studio Sprint now produces exactly 899 unique, valid terrain underlaps for its 899 real road seam edges while excluding the map perimeter.',
    '2026-07-24 15:59 EDT - Extended the outermost visible road, apron, or shoulder one meter beneath terrain and extended terrain one meter beneath that track edge, dropped both buried endpoints three meters, preserved their explicit render elevations, and added visibility-mode and non-mutation regressions.',
    '2026-07-24 15:41 EDT - Added an 8 cm outward, 4 cm downward apron/shoulder seam skirt without changing collision; cached stable top-speed physics inputs, stopped per-sample ghost array copies, moved route paint and furniture into the revision-keyed static track bundle, and limited wet-weather rebuilds to actual effective-material changes.',
    '2026-07-24 14:47 EDT - Eliminated false terrain cuts from bounding-box-only road overlap, stopped internal road-triangle diagonals from becoming seams, removed the permissive connected-seam validator, and skipped discarded full-terrain visibility scans in static Three playtests to prevent periodic driving hitches.',
    '2026-07-24 11:35 EDT - Added a revision-keyed baked-surface spatial sampler, restored five-point road-to-terrain transitions, and made dense painted terrain render from its actual heightmap cells instead of kilometer-wide interpolated rails.',
    '2026-07-24 10:59 EDT - Replaced sequential roadbed curvature mutation with a bounded symmetric solver, added asphalt/wet/gravel/dirt/snow friction-heat scaling for player and AI tires, and added a stop-wait-restart Studio Sprint terrain-hill regression.',
    '2026-07-24 09:18 EDT - Added a shared capacity-safe differential resolver for AWD center and FWD/RWD axle differentials, contact-gated player and AI tire behavior, a drivetrain unload matrix, and a real Studio Sprint WRX/WRX2 gravel-hill regression.',
    '2026-07-24 02:26 EDT - Decoded compact project documents in the local static storage server and added direct regression coverage for compressed Studio Sprint-style payloads.',
    '2026-07-24 02:08 EDT - Exposed live grade percent and grade-force pounds in the Race Editor debug HUD, making steep hill and downhill validation easier to compare against the corrected grade-force physics.',
    '2026-07-24 02:00 EDT - Forwarded projected AI car world coordinates, elevation, and segment into custom art shadows and geometric debug overlays, with regression coverage proving AI sprites no longer fall back to origin-like pose data.',
    '2026-07-24 01:52 EDT - Refreshed the Latest Changes status with why the physics audit is not finished, what is taking the time, the current 98% estimate, and the remaining 45 to 90 minute validation window.',
    '2026-07-24 01:51 EDT - Corrected player and AI hill grade force to use the gravitational component along the road angle instead of treating rise/run grade as sine, slightly reducing unrealistic harshness on steep hill climb and downhill brake tests.',
    '2026-07-24 01:44 EDT - Stored landing impact at touchdown and taught race diagnostics to use that preserved value, preventing hard landings from being reported as stable just because vertical velocity was already zeroed.',
    '2026-07-24 01:36 EDT - Bounded third-person playtest camera-yaw changes with a drift-aware smoother and added a regression proving sudden drift direction changes cannot snap the camera around the car before converging back to travel yaw.',
    '2026-07-24 01:28 EDT - Updated Latest Changes with a clearer completion note explaining why the physics audit is not done, what is taking the time, the current 97% estimate, and the 45 to 90 minute remaining validation window.',
    '2026-07-24 01:27 EDT - Consolidated race tire load-sensitivity math so the Race Editor force model and 3D vehicle chassis use the same loose-surface curve instead of separate near-match formulas.',
    '2026-07-24 01:20 EDT - Mirrored the 1200 HP dirt drive-force rejection proof into the browser Playwright race-feel spec, so the supported-platform app-path gate checks demanded force, applied force, rejected force, and applied-to-demanded ratio.',
    '2026-07-24 01:12 EDT - Strengthened the dedicated race-physics feel suite so 1200 HP dirt cases must prove large absolute drive-force rejection and a low applied-to-demanded force ratio, not just high traction demand ratios.',
    '2026-07-24 01:04 EDT - Fixed AI airborne fallback physics so AI cars explicitly marked airborne but missing initialized 3D wheel contacts cannot drive or brake against static fallback tire loads before contact telemetry catches up.',
    '2026-07-24 00:56 EDT - Routed AI longitudinal, brake, and lateral tire limits through the same deterministic bump normal-load scaling as player cars, with coverage proving bumpy gravel varies per-wheel load without adding net tire load.',
    '2026-07-24 00:47 EDT - Added live drive-force demand/applied/loss telemetry to the Race Editor debug HUD, making 1200 HP dirt runs show how much engine force is being rejected by tire saturation and combined-slip limits.',
    '2026-07-24 00:39 EDT - Added live contact telemetry to the Race Editor debug HUD, showing tire contact, driven-wheel load, and front/rear axle contact percentages so hill crests, jumps, and partial-unload bouncy behavior can be diagnosed while driving.',
    '2026-07-24 00:31 EDT - Cleared third-person camera anchor, placement, chase-distance, and smoothing history during route-center respawn so stale camera state cannot survive a reset and make the car appear to pivot around an old chase point.',
    '2026-07-24 00:22 EDT - Added third-person chase-distance telemetry and a camera jitter regression that alternates terrain safety pull-ins with recovery frames, proving hill and bump avoidance does not snap the chase camera back out abruptly.',
    '2026-07-24 00:13 EDT - Added brake/ABS diagnostics to race playtest telemetry, showing requested brake force, combined applied brake force, ABS intervention, and max wheel lock so loose-surface braking and trail-braking can be diagnosed while driving.',
    '2026-07-24 00:06 EDT - Added automatic-shift diagnostics to player drivetrain telemetry and race-feel coverage, exposing upshift/downshift thresholds plus shift torque cut so 1200 HP dirt validation can prove shifting is not hiding tire saturation.',
    '2026-07-24 00:02 EDT - Smoothed the engine drive-force transition from low-speed torque-limited launch to power-limited acceleration, added continuity coverage around the old 8 m/s threshold, and exposed the power-limit blend in drivetrain telemetry.',
    '2026-07-23 23:58 EDT - Added drivetrain-softening diagnostics to race telemetry and regressions, exposing applied drive demand, traction-control cut, and drivetrain unload so 1200 HP dirt tests prove torque is not silently being tamed before tire saturation.',
    '2026-07-23 23:52 EDT - Exposed load-sensitive effective tire friction coefficients in playtest telemetry and race-feel regressions, so dirt/gravel 1200 HP validation now proves the car is using a loose-surface mu envelope instead of hidden asphalt-like grip.',
    '2026-07-23 23:47 EDT - Added live third-person camera diagnostics to the race playtest telemetry HUD, showing chase-anchor lag, body/world mismatch, and screen projection error so mid-track pivoting can be diagnosed while driving.',
    '2026-07-23 23:45 EDT - Updated the Latest Changes status header with why the physics audit is not finished, the current completion estimate, the remaining validation risk, and the expected time left before the audit can be called complete.',
    '2026-07-23 23:42 EDT - Tightened the third-person drift camera anchor so high-slip dirt slides keep a small bounded lateral lag for drift readability without letting the car swing multiple meters around an external chase point mid-track.',
    '2026-07-23 23:38 EDT - Added third-person car render-position diagnostics and tightened the mid-track stale-body regression so the race playtest proves the drawn car follows live worldX/worldZ anywhere on the route, not only at the starting line.',
    '2026-07-23 23:35 EDT - Extended the browser race-physics Playwright spec to cover the live update-loop 3D contact sync, proving updatePlaytest keeps body, vehicle3d position, and wheel contacts attached to live worldX/worldZ after stale body coordinates are injected.',
    '2026-07-23 23:31 EDT - Fixed the 3D vehicle contact update order by syncing vehicle3d planar position to live worldX/worldZ before stepping wheel contacts, so stale body coordinates cannot make the solver sample tires behind the actual car before the post-movement render sync.',
    '2026-07-23 23:26 EDT - Refined the 3D tire-envelope regressions to prove the stock AWD dirt partial-throttle case stays under driven-wheel longitudinal capacity while 1200 HP dirt builds exceed drive and 3D chassis limits; lateral tire saturation on dirt remains allowed while turning.',
    '2026-07-23 23:22 EDT - Extended the browser third-person render-position Playwright coverage to assert 3D wheel contact points translate with the live worldX/worldZ body sync, so app-path tire art and geometric overlays cannot lag behind the corrected car position.',
    '2026-07-23 23:19 EDT - Tightened the third-person render-position fix by translating 3D wheel contact points whenever the live planar body position is resynced to worldX/worldZ, preventing tire art and geometric overlays from lagging behind the corrected car body.',
    '2026-07-23 23:15 EDT - Added browser Playwright coverage for the third-person render-position fix: the app-path test now forces stale bodyX/bodyZ and proves the drawn car center follows live worldX/worldZ, matching the local start-line and mid-track render regressions.',
    '2026-07-23 23:12 EDT - Mirrored the 3D friction-circle saturation proof into the browser race-physics Playwright spec, so the supported-platform app-path gate now checks 1200 HP dirt traction against aggregate drive demand, driven-wheel usage, and 3D chassis tire-envelope telemetry.',
    '2026-07-23 23:10 EDT - Tightened the 3D chassis tire envelope for the race physics audit: combined over-limit wheel usage now reduces the 3D friction-circle force scale, and the 1200 HP dirt regressions prove the chassis solver reports tire saturation instead of only aggregate drive over-demand.',
    '2026-07-23 23:07 EDT - Fixed third-person car render position drift by making rendered car and camera anchors use live worldX/worldZ instead of stale bodyX/bodyZ, while keeping 3D body height/pitch/roll; start-line and mid-track regressions now prove the third-person car matches actual session position.',
    '2026-07-23 22:59 EDT - Completed a wheel-level telemetry wiring pass for the race physics audit: player engine-drive diagnostics now include per-wheel longitudinal usage, lateral usage, friction usage, and remaining lateral limit, with fast and browser race-feel regressions asserting driven tires exceed the force envelope in 1200 HP dirt cases.',
    '2026-07-23 22:56 EDT - Expanded race physics feel coverage across AWD, RWD, and FWD 1200 HP dirt cases, proving each drivetrain remains traction-saturated with severe loose-surface power overuse while the third-person camera stays travel-yaw aligned.',
    '2026-07-23 22:43 EDT - Tightened the race physics feel regressions so both the fast local suite and browser Playwright spec prove the projected third-person chase camera is placed behind travel/camera yaw, not body yaw, once a 1200 HP dirt slide is developed.',
    '2026-07-23 22:40 EDT - Added explicit merge-workflow gates for race physics feel validation: the unit job now runs npm run test:race-physics-feel, and a Playwright Docker job runs npm run test:race-physics-feel:browser on a supported platform.',
    '2026-07-23 22:37 EDT - Added npm run test:race-physics-feel:browser as the narrow supported-platform Playwright command for the app-path race physics feel proof; it remains blocked locally on Android by Playwright Unsupported platform: android while the fast local suite passes.',
    '2026-07-23 22:35 EDT - Added npm run test:race-physics-feel as a fast local validation command for the 1200 HP dirt traction, stock comparison, and projected third-person drift-camera regressions; the script passed locally in about 40 seconds.',
    '2026-07-23 22:33 EDT - Aligned the browser-level Playwright race-physics feel regression with the fast local suite by adding projected third-person camera-anchor checks plus drive-demand and post-peak traction assertions for the 1200 HP dirt cases.',
    '2026-07-23 22:30 EDT - Verified the browser-level race physics feel regression is blocked locally by Playwright Unsupported platform: android, then re-ran the fast local feel suite as fallback proof: all three 1200 HP dirt/camera/stock-comparison regressions passed in about 40 seconds.',
    '2026-07-23 22:26 EDT - Strengthened the fast race-physics feel regression with a projected third-person camera-anchor check, proving live 1200 HP dirt drift lags laterally instead of rendering as a body-locked orbit while camera yaw stays near travel direction.',
    '2026-07-23 22:23 EDT - Strengthened the fast race-physics feel regression with a stock-versus-1200 HP AWD dirt comparison, proving the stock car stays under tire capacity while the overpowered partial-throttle build still saturates wheelspin and slides.',
    '2026-07-23 22:21 EDT - Added a fast dedicated race-physics feel regression proving a 1200 HP AWD dirt steering case stays massively traction-limited, rotates hard, barely accelerates versus asphalt, and keeps third-person camera yaw close to travel direction.',
    '2026-07-23 22:17 EDT - Continued the race physics audit by exposing 3D tire-limit, load-sensitivity, and friction-circle telemetry in the normal playtest HUD so high-power dirt slides can be diagnosed from visible tire capacity rather than feel alone.',
    '2026-07-23 22:13 EDT - Continued the race physics audit by adding load-sensitive tire force envelopes to the 3D chassis solver, so outside tires under anti-roll or cornering load do not keep a linear normal-load-to-grip advantage.',
    '2026-07-23 22:08 EDT - Continued the race physics audit by making 3D anti-roll tuning transfer normal load across each axle before tire force limits are calculated, while capping transfer by available inside-wheel load so total chassis support stays stable.',
    '2026-07-23 22:01 EDT - Continued the race physics/camera audit by smoothing third-person chase-distance recovery after terrain safety pull-ins, keeping immediate terrain avoidance while reducing hill and bump zoom jitter.',
    '2026-07-23 21:25 EDT - Added adaptive AWD center-differential coupling so torque can shift toward the axle with available grip, exposed center diff lock in Race and Car Editor tuning, and retuned tarmac tires to lose a little more grip on gravel/wet gravel while preserving matched dirt-tire hill climbs.',
    '2026-07-23 21:13 EDT - Continued the tire/surface physics audit by fixing the Car Editor default tire label to follow the selected setup compound and adding a snow-condition ordering regression so ice, slush, deeper snow, and dusting keep the expected relative grip.',
    '2026-07-23 21:10 EDT - Added wheel terrain-region codes to the Race Editor telemetry HUD alongside surface codes, making it visible when a tire is on road, margin, transition, or off-road while diagnosing tarmac-versus-edge spin reports.',
    '2026-07-23 21:07 EDT - Expanded race physics regression coverage with a physical-gamepad full-trigger 8 MPH tarmac no-donut test and added the moving 1200 HP AWD dirt-slide camera case to the browser-level Playwright spec.',
    '2026-07-23 21:02 EDT - Tightened third-person camera yaw during moving 1200 HP AWD dirt slides so severe medium-speed body-vs-travel drift stays visually anchored near actual travel direction instead of feeling like the car pivots around the camera.',
    '2026-07-23 20:57 EDT - Tightened the low-speed tarmac steering fix by keeping AWD/FWD rear breakaway suppressed on high-grip pavement, added a mobile full-lock throttle regression that must understeer before leaving the road, and kept the WRX top-speed target in the real 143-146 mph manual band.',
    '2026-07-23 20:40 EDT - Fixed low-speed tarmac steering feel by delaying full yaw authority until tire lateral force has enough speed to match it, added an 8 MPH no-donut regression, and split WRX stock acceleration targets by transmission so the default SPT automatic is not judged against the quicker manual band.',
    '2026-07-23 16:22 EDT - Added a browser-level Playwright race physics feel regression for the loaded app path and verified the local static server responds at http://127.0.0.1:4173 for manual playtesting.',
    '2026-07-23 16:19 EDT - Checked third-person camera anchor lag during a live 1200 HP RWD dirt steering spin; anchor offset stayed under roughly 0.16 m while camera yaw stayed near travel direction, confirming the remaining pivot fix belonged in yaw/traction rather than placement.',
    '2026-07-23 16:16 EDT - Added live analog-throttle dirt coverage showing a stock AWD car remains usable with partial throttle while a 1200 HP AWD tune stays traction-limited, spins hard, and keeps the third-person camera close to travel yaw.',
    '2026-07-23 16:07 EDT - Trimmed redundant dynamic hill and downhill brake test cases while preserving all-surface static margins and live snow, gravel, asphalt, and downhill edge-case coverage, cutting the focused grade/brake runtime to about 49 seconds.',
    '2026-07-23 16:01 EDT - Continued the race physics/camera audit by adding a live 1200 HP RWD dirt steering-spin regression that keeps third-person camera yaw close to travel direction while wheelspin and yaw instability stay high.',
    '2026-07-23 15:56 EDT - Continued the race physics/camera audit by making severe low-speed power spins pull the third-person camera toward actual travel yaw, preventing high-power loose-surface burnouts from feeling body-locked.',
    '2026-07-23 16:06 EDT - Updated Car Editor Load Preset into a real E through A class by Race, Rally, Offroad, and Drift preset matrix, and capped automatic brake-to-reverse behavior so reverse stays a crawl instead of outrunning forward launch.',
    '2026-07-23 15:33 EDT - Added force-balance coverage for 1, 5, 10, and 15 degree hill climbs and downhill braking across asphalt, dirt, gravel, and snow tire/surface pairings.',
    '2026-07-23 15:24 EDT - Continued the race physics audit by treating unknown tire temperature as ambient instead of 0F, restoring live stock asphalt grip and bringing WRX playtest braking close to documented real-world stopping targets.',
    '2026-07-23 15:16 EDT - Continued the race physics audit by removing hidden straight-line scrub braking from rear breakaway memory, lowering the low-friction tire floor, and keeping top-speed caps from acting as brakes while coasting or stopping.',
    '2026-07-23 15:09 EDT - Split MIDI Record tuning into its own dedicated Tuning panel and added adjacent Tuning controls in both portrait and landscape so Record Settings stays compact.',
    '2026-07-23 15:08 EDT - Continued the race physics audit by making modified-car top-speed estimates and runtime caps respect driven-tire traction on loose surfaces, so overpowered RWD dirt builds no longer report asphalt-like reachable speeds.',
    '2026-07-23 14:56 EDT - Continued the race physics audit by making stock Car Editor Summary Sheet lateral grip and braking rows derive from documented real-world targets on clear asphalt, while keeping modified and loose-surface builds on the physics estimator.',
    '2026-07-23 14:47 EDT - Continued the race physics audit by making automatic player and AI gearboxes upshift out of stale over-revving low gears during braking or coasting, preventing unrealistic engine-braking spikes when speed state is high.',
    '2026-07-23 14:39 EDT - Continued the race physics audit by making Summary Sheet braking distances integrate tire-limited brake force with aero drag and rolling resistance instead of using a constant brake-only deceleration estimate.',
    '2026-07-23 14:32 EDT - Added OK and Cancel dismissal actions to MIDI landscape Record Virtual/Settings panels and consolidated the better per-string tuning picker into Record Settings.',
    '2026-07-23 14:31 EDT - Continued the race physics audit by making loose dirt and gravel road surfaces increase rolling resistance for player, AI, and shared resistance calculations instead of behaving like asphalt unless the car left the road.',
    '2026-07-23 14:23 EDT - Continued the race physics audit by routing AI target speeds and longitudinal speed caps through the same stock-governor versus physical power/drag/gearing top-speed limits as player playtest cars.',
    '2026-07-23 14:14 EDT - Continued the race physics audit by separating real stock top-speed governors from physical power/drag/gearing limits, so modified high-power builds are no longer capped by inherited stock WRX top-speed values in Summary Sheet or playtest telemetry.',
    '2026-07-23 13:55 EDT - Restored the MIDI Song screen landscape zoom slider in the shared lower-right zoom slot beneath the song mode rail.',
    '2026-07-23 14:02 EDT - Continued the race physics audit by correcting the default 2022 WRX Summary Sheet top speed to real transmission-specific targets: 134 mph for the default SPT automatic and about 145 mph for the manual instead of the old 160 mph value.',
    '2026-07-23 13:48 EDT - Continued the race physics audit by making unpainted dirt and gravel shoulders/transitions inherit the authored road surface, so Studio Sprint loose-surface tire telemetry does not silently become asphalt near the road edge.',
    '2026-07-23 13:36 EDT - Continued the race physics audit by checking Studio Sprint and built-in loose-surface route grades, then strengthening dirt tire loose-surface grip so the default 271 hp WRX can keep climbing steep dirt and gravel grades on the correct tires.',
    '2026-07-23 13:27 EDT - Continued the race physics audit by normalizing procedural bump normal-load scales so rough loose surfaces vary individual wheel grip without adding net tire load and phantom traction.',
    '2026-07-23 13:24 EDT - Continued the race physics audit by adding yaw acceleration from actual left-right longitudinal tire-force imbalance, so locked diffs and split-grip throttle rotate the car from drivetrain force instead of only generic surface pull.',
    '2026-07-23 13:21 EDT - Continued the race physics audit by making traction control commit its torque cut from the final load-transfer traction limit instead of the preliminary estimate, keeping high-power dirt launches tied to the actual available driven grip.',
    '2026-07-23 13:17 EDT - Continued the race physics audit by normalizing saved car tuning so rev limits cannot remain below redline, keeping automatic transmission shift thresholds stable for older or imported cars after redline edits.',
    '2026-07-23 13:10 EDT - Continued the race physics audit by applying terrain grip scale to fallback wheel-surface friction, preventing off-road fallback contact from reporting asphalt-like average surface grip when no full surface-model sample is present.',
    '2026-07-23 13:07 EDT - Continued the race physics audit by making the 3D chassis contact solver reduce wheel force envelopes when per-wheel tire usage is post-peak, so overloaded tires no longer shake the body with peak grip after gameplay traction has fallen away.',
    '2026-07-23 13:04 EDT - Continued the race physics audit by making live tire wear use each wheel\'s own compound wear rate instead of a car-wide average, so mixed-compound setups damage the tire that is actually fitted and slipping.',
    '2026-07-23 13:01 EDT - Continued the race physics audit by carrying car-level traction-control tuning into playtest sessions, so TC-off high-power dirt builds are not silently re-enabled by the runtime input defaults.',
    '2026-07-23 12:57 EDT - Continued the race physics audit by reducing low-speed yaw spin recovery while active high-power dirt wheelspin is overloading the driven tires, preserving recovery after lift-off without hidden stability-control damping under throttle.',
    '2026-07-23 12:52 EDT - Continued the race physics/camera audit by making third-person camera yaw blend toward travel direction during low-speed high-slip dirt launches, reducing the body-locked pivot feel when overpowered cars spin the tires.',
    '2026-07-23 12:49 EDT - Continued the race physics audit by making Car Editor Summary Sheet acceleration, braking, lateral grip, and top-speed estimates accept surface/weather context, so high-power dirt setups show loose-surface traction loss instead of asphalt-only performance.',
    '2026-07-23 12:45 EDT - Continued the race physics audit by treating stale automatic downshift RPMs as invalid after redline/rev-limit edits, keeping edited-redline upshifts from landing below the downshift point and hunting gears.',
    '2026-07-23 12:37 EDT - Continued the race physics audit by making Summary Sheet braking distances use the same tire-limited brake force and load-transfer model as runtime, so extra brake pressure cannot report impossible stopping distances once tires are saturated.',
    '2026-07-23 12:33 EDT - Continued the race physics audit by aligning AI tire heat telemetry with the same per-wheel temperatures used for traction and rolling resistance in that physics step, while carrying the heat update into the next step.',
    '2026-07-23 12:29 EDT - Continued the race physics/camera audit by adding a third-person chase anchor that follows forward motion quickly but lets lateral drift lag visibly, reducing the body-locked pivot/orbit feel during slides.',
    '2026-07-23 12:18 EDT - Continued the race physics audit by giving AI cars live per-wheel tire temperature and hot-pressure grip loss, so sustained high-power dirt wheelspin degrades AI rear traction like the player car.',
    '2026-07-23 12:04 EDT - Continued the race physics audit by deriving automatic upshift and downshift thresholds from the tuned redline and rev limit, preventing stale preset shift RPMs after transmission tuning changes.',
    '2026-07-23 11:57 EDT - Continued the race physics audit by sending combined-slip-reduced brake and drive force maps into player and AI 3D chassis physics, so trail braking while cornering cannot still shake the body with raw over-limit tire force.',
    '2026-07-23 11:47 EDT - Continued the race physics audit by capping partial limited-slip differential axle traction to the force that its actual split-grip torque share can support, preventing the weaker driven tire from being overdriven.',
    '2026-07-23 11:38 EDT - Continued the race physics audit by making player and AI lateral grip use each wheel\'s remaining friction-circle capacity after longitudinal load, so 1200 HP RWD dirt throttle consumes rear cornering grip instead of retaining axle-average grip.',
    '2026-07-23 11:29 EDT - Continued the race physics audit by constraining 3D chassis per-wheel longitudinal and lateral tire forces to the same friction circle, preventing combined throttle-and-cornering loads from exceeding available grip.',
    '2026-07-23 11:24 EDT - Continued the race physics/camera audit by smoothing third-person camera vertical-velocity cues separately from terrain slope, reducing bump and hill jitter without losing road-elevation anticipation.',
    '2026-07-23 11:15 EDT - Continued the race physics audit by calibrating the stock Civic Type R tire grip against documented acceleration targets and removing the Summary Sheet acceleration estimator floor that made powerless cars creep forward.',
    '2026-07-23 10:59 EDT - Continued the race physics audit by separating engine torque command from driven-wheel contact load, so partial or zero driven contact no longer hides throttle demand before traction, wheelspin, and telemetry are calculated.',
    '2026-07-23 10:51 EDT - Continued the race physics audit by preserving frame-start road distance for crest detection and zeroing stale wheel-contact loads on same-frame lift-off, so steering, brake, and drive forces cannot get an extra grounded frame after a sharp drop.',
    '2026-07-23 10:45 EDT - Continued the race physics/camera audit by smoothing third-person camera road elevation, horizon, hill pitch, and near-plane boost across bump frames while still snapping on resets and large terrain discontinuities.',
    '2026-07-23 10:39 EDT - Continued the race physics audit by preserving signed AI curve acceleration through load transfer and 3D lateral tire forces, so left and right turns mirror physically instead of always loading the same side.',
    '2026-07-23 10:33 EDT - Continued the race physics audit by feeding player and AI engine-braking tire force into the 3D chassis contact solver, so lift-off/coast deceleration affects suspension pitch and wheel forces instead of only speed telemetry.',
    '2026-07-23 10:29 EDT - Continued the race physics audit by routing player and AI engine braking through differential decel force sharing, so lift-off/coast behavior can use decel lock settings instead of reusing throttle-side accel sharing.',
    '2026-07-23 10:11 EDT - Continued the race physics audit by syncing brake balance tuning with the runtime front brake bias value, so the brake screen shows the same front/rear split that the tire-limited braking model actually uses.',
    '2026-07-23 10:01 EDT - Continued the race physics audit by reducing direct upward terrain-follow velocity in the 3D suspension model, so small fast dirt bumps are damped by the springs instead of launching the chassis into repeated bounce.',
    '2026-07-23 09:53 EDT - Continued the race physics audit by reducing bicycle-model yaw correction during large high-speed body-vs-travel slides, so loose-surface steering no longer regains artificial pivot authority while the car is still sideways.',
    '2026-07-23 09:42 EDT - Continued the race physics audit by charging tire slip wear to the wheel that is actually slipping, so rear-drive 1200 HP dirt burnouts destroy rear tires without falsely wearing the non-driven fronts.',
    '2026-07-23 09:29 EDT - Continued the race physics audit by removing hidden traction-control grip from post-peak tire efficiency, so TC helps by cutting torque while sustained 1200 HP dirt pulls still overload the tires and show residual wheelspin.',
    '2026-07-23 09:19 EDT - Added delayed route-center respawn coverage so airborne cars with stale high chassis height are reset back to realistic ride height with rebuilt suspension load instead of reappearing above the track.',
    '2026-07-23 11:04 EDT - Continued the race physics audit by reducing how much aero downforce becomes usable tire load on loose surfaces, keeping physical downforce telemetry while preventing high-speed dirt and gravel from feeling artificially planted.',
    '2026-07-23 10:56 EDT - Continued the race physics/camera audit by making large body-vs-travel slip angles reduce hidden velocity alignment and yaw assist, so sustained dirt slides keep drift separation instead of snapping travel yaw back to the car body.',
    '2026-07-23 10:48 EDT - Continued the race physics audit by removing the hidden drive-traction boost from locked differentials and centered AWD, leaving traction gains to the actual per-axle differential limit model instead of adding extra fantasy grip.',
    '2026-07-23 10:40 EDT - Continued the race physics audit by steepening tire load sensitivity, especially on loose surfaces, so weight transfer and aero load no longer grant overly efficient grip to overloaded tires during high-power launches.',
    '2026-07-23 10:32 EDT - Continued the race physics/camera audit by keeping third-person chase camera placement tied to sampled ground elevation instead of chassis body height, reducing hill/airborne zoom and pivot artifacts during drift.',
    '2026-07-23 10:24 EDT - Continued the race physics audit by initializing 3D vehicle wheel compression and normal load directly from spawn/respawn geometry, preventing reset states from starting with contact-marked wheels that still have zero suspension load.',
    '2026-07-23 10:16 EDT - Continued the race physics audit by making non-ABS locked braking fall below peak tire force on loose surfaces, with per-wheel sliding-efficiency telemetry so lockup no longer preserves ideal threshold braking force.',
    '2026-07-23 10:08 EDT - Continued the race physics audit by preventing combined-slip longitudinal loss from double-counting pure straight-line wheelspin, so post-peak drive traction handles launches while combined-slip loss is reserved for meaningful lateral tire usage.',
    '2026-07-23 10:00 EDT - Continued the race physics audit by making the Car Editor Summary Sheet acceleration estimator use the same post-peak traction helper as runtime playtest physics, removing a stale private residual-grip shortcut for high-power launches.',
    '2026-07-23 09:52 EDT - Continued the race physics audit by lowering residual post-peak longitudinal grip during severe loose-surface wheelspin, so full-throttle 1200 HP dirt launches lose more usable drive force instead of retaining asphalt-like traction authority.',
    '2026-07-23 09:44 EDT - Continued the race physics/camera audit by reducing bicycle-model yaw correction and passive yaw damping during severe loose-surface power overuse, so 1200 HP dirt launches slide and rotate instead of being held by hidden pivot assist.',
    '2026-07-23 09:36 EDT - Continued the race physics audit by feeding AI per-wheel drive, brake, longitudinal-slip, and lateral-slip telemetry into the 3D chassis contact solver so AI suspension/contact behavior reflects the same tire forces used by gameplay physics.',
    '2026-07-23 09:28 EDT - Continued the race physics/camera audit by making third-person camera yaw lean harder toward velocity yaw during high-slip drift, keeping the chase view behind actual travel instead of retaining body-yaw orbit bias.',
    '2026-07-23 09:20 EDT - Updated the Car Editor tuning Weight screen so the former weight-balance action opens a Weight panel with both vehicle-mass and front-balance sliders wired into race tuning.',
    '2026-07-23 09:12 EDT - Continued the race physics audit by converting normalized ride-height and suspension-travel tuning into realistic physical meters, fixing WRX defaults that displayed near 20 inches high and preventing route-center respawns from preserving stale high body height.',
    '2026-07-23 09:04 EDT - Continued the race physics audit by giving AI cars persistent longitudinal wheelspin relaxation, so high-power loose-surface AI traction builds and recovers over time like the player instead of snapping to an instant algebraic grip cap.',
    '2026-07-23 08:56 EDT - Continued the race physics/camera audit by removing lagged third-person chase placement yaw, keeping the chase camera directly behind live travel camera yaw during drifts instead of sliding beside the car.',
    '2026-07-23 08:48 EDT - Continued the race physics audit by exposing tire contact scale in live playtest slip telemetry and adding uphill, downhill, and airborne grade-force coverage so hill gravity cannot affect a fully airborne car.',
    '2026-07-23 08:40 EDT - Continued the race physics audit by increasing baseline loose-surface bump chatter and low-speed normal-load variation, so dirt and gravel traction is no longer unrealistically smooth when no explicit segment bumpiness is authored.',
    '2026-07-23 08:32 EDT - Continued the race physics audit by sharpening tire compound mismatch on loose surfaces, so tarmac/rain/drift tires lose much more dirt, gravel, mud, snow, and slush grip while rally/offroad/snow compounds keep their intended advantage.',
    '2026-07-23 08:24 EDT - Continued the race physics audit by reducing how much aero downforce becomes usable tire load on loose surfaces, so high-speed dirt and gravel cars do not become asphalt-planted while drag and physical downforce telemetry remain intact.',
    '2026-07-23 08:16 EDT - Continued the race physics audit by carrying wheelspin drivetrain unload through throttle lift-off, so high-power dirt wheelspin recovers with engine/driveline inertia instead of instantly snapping back to road-coupled RPM.',
    '2026-07-23 08:08 EDT - Continued the race physics audit by reducing grounded yaw-rate correction and passive yaw damping when tires are overloaded, so high-power dirt slides are not pulled back into a hidden bicycle-model pivot.',
    '2026-07-23 08:00 EDT - Continued the race physics audit by gating 3D vehicle yaw control to wheel contact, so fully airborne cars preserve yaw angular momentum instead of blending toward a grounded control yaw.',
    '2026-07-23 07:52 EDT - Continued the race physics audit by letting severe throttle wheelspin break out of launch steering alignment, so overpowered dirt launches can yaw and drift even while the car is still crawling behind the start line.',
    '2026-07-23 07:44 EDT - Continued the race physics audit by letting live tire temperature raise effective tire pressure per wheel, so sustained wheelspin narrows grip through hot pressure while also increasing heat and wear telemetry.',
    '2026-07-23 07:36 EDT - Continued the race physics audit by gating effective handbrake slip to rear-axle contact, so rear-lock yaw and grip loss do not persist when rear wheels are unloaded over crests.',
    '2026-07-23 07:28 EDT - Continued the race physics audit by letting locked/LSD drivetrain force sharing route zero tire force to unloaded driven wheels, keeping per-wheel telemetry aligned with actual tire capacity over crests.',
    '2026-07-23 07:20 EDT - Continued the race physics audit by making ABS less perfect on loose surfaces, targeting a lower effective brake cap with more pulse leak so snow and dirt braking do not behave like ideal threshold braking.',
    '2026-07-23 07:12 EDT - Continued the race physics audit by making live traction-control torque cuts respond over time instead of applying a perfect one-frame clamp, with telemetry for target versus applied TC intervention.',
    '2026-07-23 07:04 EDT - Continued the race physics audit by reducing travel/body velocity alignment when tires are over the friction circle, so overpowered dirt slides keep more drift separation instead of being artificially pulled back into a pivot.',
    '2026-07-23 06:56 EDT - Continued the race physics audit by allowing analytic lateral load transfer to fully unload inside tires without phantom minimum grip while preserving total axle load.',
    '2026-07-23 06:48 EDT - Continued the race physics audit by sharpening loose-surface post-peak tire falloff and lowering residual driven traction when overpowered cars exceed the friction circle without traction control.',
    '2026-07-23 06:40 EDT - Continued the race physics audit by gating 3D suspension pitch/roll correction to actual wheel contact, so airborne cars preserve pitch and roll angular momentum instead of self-leveling or tilting from tire-control forces in mid-air.',
    '2026-07-23 06:32 EDT - Continued the race physics audit by making AI lateral combined-slip usage use dynamic transferred wheel loads, and added a crest lift-off regression so airborne cars preserve turn yaw momentum without adding steering yaw after the wheels unload.',
    '2026-07-23 06:24 EDT - Continued the race physics/camera audit by separating live third-person camera yaw from a smoothed chase-placement yaw, reducing orbit-like camera snaps during drift while keeping the view aligned to travel direction.',
    '2026-07-23 06:16 EDT - Continued the race physics audit by making dynamic player tire loads use tire-force longitudinal acceleration instead of raw pre-combined acceleration, preventing raw engine demand from inflating cornering load on overpowered dirt launches.',
    '2026-07-23 06:08 EDT - Continued the race physics audit by giving AI braking the same preliminary tire-limited load-transfer pass as the player, with brake-state telemetry for loose-surface parity checks.',
    '2026-07-23 06:00 EDT - Continued the race physics audit by basing player brake load transfer on preliminary tire-limited braking force instead of pedal demand, so loose-surface braking only shifts weight from deceleration the tires can actually generate.',
    '2026-07-23 05:52 EDT - Continued the race physics audit by basing player and AI drive load transfer on a preliminary tire-limited applied force instead of raw engine demand, so overpowered dirt launches cannot gain fantasy rear load from throttle alone.',
    '2026-07-23 05:44 EDT - Continued the race physics audit by making player longitudinal speed integrate the combined-slip-applied tire force directly, matching AI instead of applying raw force plus a separate post-correction.',
    '2026-07-23 05:36 EDT - Continued the race physics audit by adding aerodynamic downforce back onto known 3D contacted wheel loads for player and AI tire capacity while keeping airborne wheels unloaded.',
    '2026-07-23 05:28 EDT - Continued the race physics audit by correcting road-car frontal-area and rolling-resistance estimates, keeping untouched stock top-speed summaries on documented targets while modified power curves use physical drag-limited top speed.',
    '2026-07-23 05:20 EDT - Continued the race physics audit by making AI combined-slip and lateral telemetry honor resolved wheel contact loads, so partially airborne AI cars no longer treat unloaded wheels as available lateral grip.',
    '2026-07-23 05:12 EDT - Continued the race physics audit by making player and AI drive force plus engine braking use driven-wheel load scale instead of total wheel contact count, so loaded driven tires keep drivetrain authority over crests.',
    '2026-07-23 05:04 EDT - Continued the race physics audit by removing an extra contact-count brake-force scale after per-wheel normal-load limits are applied, so partially airborne braking uses the loaded tires instead of being double-penalized.',
    '2026-07-23 04:56 EDT - Continued the race physics audit by separating lateral tire speed buildup from aggregate wheel-count contact scaling, so partially loaded axles use their actual normal loads instead of an extra arbitrary contact penalty.',
    '2026-07-23 04:48 EDT - Continued the race physics audit by making playtest lateral state come from the car world-position route projection instead of a tiny synthetic drift accumulator, so road view, hazards, and fallback car placement track real slide offset.',
    '2026-07-23 04:40 EDT - Continued the race physics audit by making zero-load wheels contribute zero tire capacity/usage and gating direct steering yaw authority to front-axle contact, so cresting rear-only contact cannot steer the car in mid-air.',
    '2026-07-23 04:32 EDT - Continued the race physics audit by making driven traction limits differential-aware, so open diffs are limited by the low-grip drive wheel while locked diffs can use the grippier tire.',
    '2026-07-23 04:24 EDT - Continued the race physics audit by aligning the acceleration estimator with runtime launch RPM and drive calibration, then retuning stock WRX/BRZ/Civic values against documented acceleration and trap-speed ranges.',
    '2026-07-23 04:16 EDT - Continued the race physics audit by correcting the stock WRX drag coefficient from an unrealistic 0.08 to a road-car value and adding runtime aero-resistance coverage.',
    '2026-07-23 04:08 EDT - Continued the race physics audit by making AI longitudinal tire grip use the same per-wheel compound, pressure, weather, terrain, and damage resolver as player tire grip.',
    '2026-07-23 04:00 EDT - Continued the race physics audit by separating actual 3D wheel loads from analytic reference loads so player and AI tire load sensitivity stays active while contact loads remain authoritative.',
    '2026-07-23 03:52 EDT - Continued the race physics audit by making AI longitudinal traction, braking, grade, and rolling resistance honor known 3D wheel contact loads instead of analytic fallback grip.',
    '2026-07-23 03:44 EDT - Continued the race physics audit by making the 3D suspension body follow rising terrain contact planes so uphill wheel contact does not let the chassis sink through the sampled road height.',
    '2026-07-23 03:36 EDT - Continued the race physics audit by making known 3D wheel normal loads authoritative for player tire capacity, so cresting or zero-load wheels no longer keep static fallback grip.',
    '2026-07-23 03:28 EDT - Continued the race physics audit by exposing ABS requested/applied brake force, tire limit, and per-wheel intervention telemetry without hiding raw brake demand.',
    '2026-07-23 03:20 EDT - Continued the race physics audit by feeding raw steering geometry into the live tire solver before applying the current grip cap, avoiding a duplicate fixed-g steering limit.',
    '2026-07-23 03:12 EDT - Continued the race physics audit by removing duplicate tire-damage grip scaling so tire wear affects the damaged wheel through per-wheel grip instead of also lowering global grip.',
    '2026-07-23 03:04 EDT - Continued the race physics audit by removing duplicate tire-temperature grip scaling so heat affects each wheel once through per-wheel grip, with resolved grip telemetry.',
    '2026-07-23 02:56 EDT - Continued the race physics audit by replacing the Summary Sheet acceleration estimator fixed traction shortcut with driven-wheel load-sensitive traction and post-peak drive falloff.',
    '2026-07-23 02:48 EDT - Continued the race physics audit by applying combined-slip longitudinal force correction and telemetry to AI cars, so AI throttle and braking share tire grip with curve demand like the player.',
    '2026-07-23 02:39 EDT - Continued the race physics audit by exposing combined-slip longitudinal applied force telemetry and covering dirt trail braking so braking and cornering share finite tire grip.',
    '2026-07-23 02:31 EDT - Continued the race physics audit by feeding longitudinal wheelspin back into drivetrain RPM, so high-power loose-surface launches flare toward redline instead of staying road-speed coupled.',
    '2026-07-23 02:22 EDT - Continued the race physics audit by making combined tire slip reduce longitudinal drive/brake force too, so cornering and throttle share the same finite tire grip instead of only reducing lateral force.',
    '2026-07-23 02:14 EDT - Continued the race physics audit by preserving airborne yaw momentum and travel direction, gating tire yaw damping, spin recovery, velocity alignment, and scrub to actual wheel contact.',
    '2026-07-23 02:06 EDT - Continued the race physics audit by making manual shifts temporarily clutch-disengage engine RPM from wheel RPM, with shift clutch and torque-cut telemetry during gear changes.',
    '2026-07-23 01:58 EDT - Added RWD/AWD/FWD drivetrain buttons to the Car Editor Transmission tuning screen so drivetrain can be changed alongside Manual/Auto and gearing controls.',
    '2026-07-23 01:51 EDT - Continued the race physics audit by centralizing longitudinal aero drag and rolling resistance so player, AI, and acceleration estimates use physical frontal-area speed-squared drag instead of diverging force shortcuts.',
    '2026-07-23 01:42 EDT - Continued the race physics audit by adding tire self-aligning steering correction so released steering unwinds from front slip angle instead of relying only on controller recentering.',
    '2026-07-23 01:34 EDT - Continued the race physics audit by adding longitudinal wheelspin relaxation so loose-surface tire spin builds and decays over time instead of snapping directly to each frame of drive demand.',
    '2026-07-23 01:26 EDT - Continued the race physics audit by placing the third-person chase camera behind the live travel/camera yaw during drifts instead of behind body yaw while looking along velocity yaw.',
    '2026-07-23 01:19 EDT - Continued the race physics audit by separating raw engine drive demand from traction-control-cut and final applied tire force, so overpowered dirt launches still expose demand, TC intervention, residual wheelspin, and applied-force telemetry.',
    '2026-07-23 01:11 EDT - Continued the race physics audit by letting lateral tire force rotate the car travel vector before residual grip alignment, reducing pivot-like steering feel and exposing velocity-yaw force telemetry.',
    '2026-07-23 01:03 EDT - Continued the race physics audit by wiring normalized Car Editor spring, bump, rebound, and anti-roll tuning into the live 3D wheel-contact solver, preserving explicit physical spring/damper rates, and exposing resolved suspension rates on wheel telemetry.',
    '2026-07-23 00:55 EDT - Continued the race physics audit by replacing fixed tire-pressure penalties with compound/surface pressure targets, tire-size grip effects, pressure-driven rolling resistance, heat, wear, and live pressure telemetry.',
    '2026-07-23 00:48 EDT - Continued the race physics audit by adding driven-wheel engine braking for player and AI cars, so lift-off coasting in gear decelerates through drivetrain drag and consumes tire capacity instead of behaving like neutral rolling drag.',
    '2026-07-23 00:41 EDT - Continued the race physics audit by adding yaw inertia and front/rear lateral-force yaw moments, so steering and drift rotation are influenced by car mass distribution instead of only chasing a target bicycle yaw rate.',
    '2026-07-23 00:34 EDT - Continued the race physics audit by adding tire load sensitivity to player and AI drive, brake, and lateral tire limits, so weight transfer increases absolute tire capacity without unrealistically preserving the same grip coefficient under heavy load.',
    '2026-07-23 00:27 EDT - Continued the race physics audit by adding front/rear tire slip-angle relaxation, making lateral force build over tire travel distance instead of snapping instantly to steering input, while widening longitudinal tire over-demand telemetry so 1200 HP loose-surface launches still drive the post-peak tire curve hard.',
    '2026-07-23 00:19 EDT - Continued the race physics audit by applying acceleration load transfer before player and AI drive traction is capped, so hard throttle unloads front tires, loads rear tires, affects per-wheel torque share, and shows drive normal loads in telemetry.',
    '2026-07-23 00:12 EDT - Fixed Car Editor power-curve dragging after the tuning-panel move by clearing stale tuning hit regions on redraw and making the active power-curve graph win pointer input before old slider regions can steal it.',
    '2026-07-23 00:08 EDT - Continued the race physics audit by turning authored segment bumpiness into deterministic per-wheel normal-load variation at speed, feeding rough-road load pulses into drive, brake, and lateral traction, and exposing bump load scales in telemetry.',
    '2026-07-23 00:04 EDT - Continued the race physics audit by adding an explicit slide recovery yaw moment when steering input is correcting rotation, exposing countersteer recovery telemetry, and covering recovery steering against steering that feeds the spin.',
    '2026-07-22 23:56 EDT - Continued the race physics audit by applying forward braking load transfer before tire-limited brake force is capped, exposing brake normal loads in telemetry, and covering front capacity/rear unload behavior in focused braking tests.',
    '2026-07-22 23:52 EDT - Continued the race physics audit by making the stock acceleration estimator report quarter-mile time and trap speed through the shared torque/power force model, adding drivetrain-specific launch and high-speed losses, and replacing the slow full-editor stock simulator test with a deterministic sub-second validation.',
    '2026-07-22 23:46 EDT - Continued the race physics audit by constraining AI acceleration with the same engine, gearing, wheel-load, aero drag, grade, and post-peak traction limits as player cars, and expanded telemetry HUD readouts for drive demand, tire health, aero load, power-yaw instability, and AI traction demand.',
    '2026-07-22 23:36 EDT - Continued the race physics audit by distinguishing uninitialized 3D wheel normal loads from real zero-load contacts, preserving launch drivetrain coupling while still letting genuinely unloaded wheels free-rev.',
    '2026-07-22 23:31 EDT - Continued the race physics audit by factoring engine drive force into torque-limited and power-limited components, exposing engine-drive telemetry for limiter/shift/traction cuts, and testing high-speed acceleration as power-limited.',
    '2026-07-22 23:29 EDT - Continued the race physics audit by feeding live tire temperature into wear, exposing tire health in telemetry, and covering high-power dirt wheelspin heating and degrading rear tires over sustained abuse.',
    '2026-07-22 23:26 EDT - Continued the race physics audit by making driven-wheel load coupling use actual 3D suspension normal force, so zero-load contacted wheels free-rev instead of receiving static fallback grip while loaded wheels stay road-coupled.',
    '2026-07-22 23:24 EDT - Continued the race physics audit by making drivetrain differential tuning bias live drive force toward higher-capacity wheels on split grip and making ABS brake-lock telemetry reflect modulated brake force instead of uncapped pedal request.',
    '2026-07-22 23:20 EDT - Continued the race physics audit by moving Car Editor aero from a hidden grip multiplier into speed-squared front/rear downforce added to wheel normal loads, with live telemetry for aero load and per-wheel normal force.',
    '2026-07-22 23:15 EDT - Continued the race physics audit by adding a surface-aware post-peak tire load curve so combined throttle/brake/steering demand rolls off usable lateral grip earlier on loose surfaces, with telemetry for front and rear post-peak grip.',
    '2026-07-22 23:12 EDT - Tightened the Car Editor power-curve fix by allowing padded graph hits after the tuning-screen move and resetting the preview restart debounce timer on each tuning input so the preview reloads only after three idle seconds.',
    '2026-07-22 23:09 EDT - Continued the race physics audit by adding deterministic torque-overload yaw instability for overpowered loose-surface launches and routing live drive force through drivetrain and center-differential per-wheel torque shares instead of equal AWD force.',
    '2026-07-22 22:58 EDT - Started the race physics realism audit by making third-person camera yaw follow vehicle travel direction during slides, lowering loose-surface grip, removing duplicate tire-grip multiplication from drive limits, and adding post-peak wheelspin traction falloff for high-power dirt launches.',
    '2026-07-22 22:45 EDT - Fixed Car Editor Tune > Power Curve graph dragging after the menu move, and debounced Car Editor preview restarts so tuning changes reload the preview only after input has been idle for three seconds.',
    '2026-07-22 22:33 EDT - Moved Car Editor Summary Sheet under Tools, kept Power Curve under Tune, made Tools a summary-only drawer, refreshed preview sessions when tuning changes, kept preview resets from reopening menus, and made Summary Sheet acceleration use edited HP/torque curves.',
    '2026-07-22 22:15 EDT - Made Car Editor Summary Sheet acceleration respond to edited power/torque curves, expanded performance estimates so every performance tuning control has a measurable effect, added Drift tires, and added File > Load Preset for Race, Rally, Offroad, and Drift full-car tune bundles.',
    '2026-07-22 21:57 EDT - Changed Car Editor gear-ratio overflow to gesture scrolling instead of Up/Down buttons, converted tire size controls to snapping sliders, moved Final Drive/redline/rev-limit controls into Transmission, renamed the Tune menu item to Transmission, and added a Summary Sheet stats panel.',
    '2026-07-22 21:41 EDT - Made low-RPM Car Editor power-curve points easier to select, moved the RPM x1000 axis label to the left edge, removed power-curve edit-mode status text, rebuilt Tires into Tire Type and Tire Size sections with validated size choices, and matched Car Editor aero/suspension units to Race Editor tuning.',
    '2026-07-22 21:13 EDT - Added horizontal breathing room to the Car Editor power-curve graph by starting the RPM axis at zero and extending slightly past max rev so edge points are easier to edit.',
    '2026-07-22 21:12 EDT - Split Car Editor Final Drive redline and rev-limit tuning, extended the power curve graph to max rev with a dotted redline marker, updated the tach red band from tuned redline/rev limit, and added scroll controls for long gear-ratio lists.',
    '2026-07-22 21:01 EDT - Kept Car Editor portrait Menu on Tune after selecting a tuning item, and removed the remaining Tune panel title/helper copy so tuning controls start at the top.',
    '2026-07-22 21:00 EDT - Removed the focused Tune panel header and Menu > Tune helper sentence from Car Editor tuning screens, moving controls up into the freed space.',
    '2026-07-22 20:56 EDT - Removed the Tune Setup heading and background tuning summary copy from Car Editor Tune panels so only the selected tuning controls are visible.',
    '2026-07-22 20:42 EDT - Removed the Drive Setup heading and drivetrain/power/weight summary copy from the Car Editor Drive power-curve panel so the screen focuses on the curve controls.',
    '2026-07-22 19:39 EDT - Cleaned up Car Editor Tune menu labels, moved tire-size presets into Tires, and added a Gearing panel with per-gear ratio sliders for the selected transmission.',
    '2026-07-22 19:01 EDT - Fixed Car Editor Tune panel input so Manual/Auto and other focused Tune buttons respond to pointer taps, and prevented embedded preview sessions from stealing portrait menu input.',
    '2026-07-22 18:51 EDT - Changed Car Editor Tune panels to stay focused on the selected Menu > Tune item, hiding the all-tuning tab strip and making Transmission Type show selectable Manual/Auto buttons.',
    '2026-07-22 18:45 EDT - Fixed Car Editor portrait Tune submenu taps so Transmission Type closes the menu sheet and reveals the drive tuning controls instead of leaving the menu over the panel.',
    '2026-07-22 18:33 EDT - Rebuilt the Car Editor Tune panel with drivetrain buttons, per-wheel tire picks and tarmac reset, pressure/brake/gearing/diff/alignment/aero/suspension sliders, tire-size presets, redline and gear-count controls, ride-height/travel tuning, and engine SFX loudness.',
    '2026-07-22 18:17 EDT - Retuned picked engine SFX playback to follow the procedural rev curve across the full RPM range and regenerated built-in engine SFX loops as seamless versioned samples to avoid end-click crackle.',
    '2026-07-22 18:06 EDT - Fixed Race and Car Editor engine SFX loops to clean up by race-engine key, update loop pitch from live RPM, and moved transmission, drivetrain, engine SFX, and weight-balance controls into the Car Editor Tune menu so the power-curve panel has more graph room.',
    '2026-07-22 17:45 EDT - Changed Car Editor Open SFX to pick project SFX files as engine sounds, seeded editable SFX documents for the built-in engine profiles, and kept car engineProfile settings as playback fallbacks.',
    '2026-07-22 17:36 EDT - Kept the Car Editor power-curve edit row to exactly TQ, HP, Add, and Del while preserving the uppercase TQ/HP labels through the shared button renderer.',
    '2026-07-22 17:05 EDT - Simplified the Car Editor power-curve controls to TQ, HP, Add, and Del, with a fixed 1200 max graph scale and guarded point deletion.',
    '2026-07-22 16:47 EDT - Updated the Car Editor power-curve graph with shared HP/ft-lb hundreds labels on the left and RPM x1000 labels along the bottom.',
    '2026-07-22 15:31 EDT - Changed the Car Editor Drive root to jump straight to the power-curve editing screen with drivetrain buttons, weight-balance slider, engine sound controls, SFX editor access, torque/HP edit modes, add-point, and draggable curve points.',
    '2026-07-22 15:18 EDT - Changed the Car Editor Art root menu to jump straight to the body art canvas state while keeping body-panel controls available for art, reset, geom overlay, and turn-frame picks.',
    '2026-07-22 13:18 EDT - Kept the Race Editor third-person chase camera from going underground by shortening the chase distance near raised terrain and guarding the resolved camera height above local ground.',
    '2026-07-22 13:10 EDT - Made the Race Editor Skybox settings editable with project art selection, north/vertical offset sliders, reset controls, and an in-game skyline preview that uses the same skybox render path as playtest.',
    '2026-07-22 12:55 EDT - Shifted Race Editor art skyboxes upward by anchoring the source horizon in the middle third of the image, exposing more lower-ground detail during hill climbs and descents.',
    '2026-07-22 12:51 EDT - Moved the Race and Car Editor third-person chase camera farther rearward so the full car body and shadow fit more comfortably in playtest framing.',
    '2026-07-22 12:46 EDT - Restored the destination completion percentage under the Race Editor playtest time while keeping the minimap percentage and tach progress bars removed.',
    '2026-07-22 12:27 EDT - Cleaned up the Race Editor playtest HUD by changing the minimap player marker to a dot, removing completion percentages and progress bars, showing reverse speed as a positive mph value, removing tach RED/bar clutter, and moving FPS/draw-call stats behind a pause-menu Debug HUD toggle.',
    '2026-07-22 11:44 EDT - Stopped Race and Car Editor playtest Start and Select hitboxes from repainting blue editor buttons over the Level-style handheld pills, leaving the visible controls entirely owned by the shared MobileControls renderer.',
    '2026-07-22 11:37 EDT - Matched Race and Car Editor mobile playtest gamepad presentation to the Level Editor standard by always drawing the shared handheld shell and virtual D-pad, R/G, Start, and Select controls in portrait and landscape, even when a physical gamepad is connected.',
    '2026-07-22 10:36 EDT - Kept saved Race Editor playtest cars distinct by file name when duplicate car document ids exist, fixed player custom car billboards to use stable third-person chase sizing, and drew the Geom overlay even when the Three procedural car already rendered.',
    '2026-07-21 23:54 EDT - Routed projected in-game car sprites through the shared Car Editor billboard renderer, grounded default shadows to sampled terrain height, and rendered the Geom debug overlay in-game when enabled on a car.',
    '2026-07-21 23:37 EDT - Kept overridden Car Editor body art flat on the center billboard anchor over hills and forced the built-in default shadow under custom body art when no custom shadow asset is selected.',
    '2026-07-21 23:13 EDT - Placed the Car Editor Geom overlay toggle in Art -> Body next to Reset, keeping it visible on narrow panels while overlaying projected physics body and wheel geometry over custom art.',
    '2026-07-21 22:35 EDT - Fixed geometric car body pitch conversion so uphill physics pitch renders front-up in both Three and 2D procedural third-person car bodies.',
    '2026-07-21 22:24 EDT - Kept override car body graphics as upright billboards anchored to the rear of the physical car while leaving tire graphics tied to physical wheel projections.',
    '2026-07-21 22:12 EDT - Corrected Three procedural car hill pitch handling for the white reference body and stopped Car Editor preview engine/tire audio when the preview is reset or the editor exits.',
    '2026-07-21 22:03 EDT - Moved the shared Race and Car Editor third-person chase camera rearward so the full procedural vehicle geometry stays visible, and scaled override body billboards from the live projected vehicle depth.',
    '2026-07-21 21:51 EDT - Let Doodad Editor plant offsets scale to one full doodad height instead of a fixed 20m cap, with Race Editor placement and doodad document normalization honoring the deeper proportional values.',
    '2026-07-21 21:39 EDT - Restored Race Editor third-person playtest cars to use the geometric/procedural reference body and wheel placement, while keeping Car Editor artwork overrides on the compact billboard sizing path.',
    '2026-07-21 21:36 EDT - Raised Doodad Editor visual and hitbox size limits to 500m through shared doodad size constants so large scenery-scale doodads survive editing, saving, loading, and Race Editor placement.',
    '2026-07-21 20:36 EDT - Fixed in-game projected race car sprites to use the same Car Editor billboard width basis instead of widening side-angle cars from vehicle length until they could span the road.',
    '2026-07-21 20:35 EDT - Consolidated portrait editor button chrome so canvas and DOM editor controls share the same rounded RTG Studio radius, accent strip, gradient fill, and focus outline instead of drifting between square and rounded treatments.',
...
export const LATEST_CHANGES = [
    '2026-08-24 09:55 EDT - Restored continuous rolling tread marks on snow, dirt, gravel, mud, and grass by sourcing mark emission from the canonical post-step wheel/contact generation instead of stale legacy vehicle3d load flags. Genuine airtime still breaks ribbons, asphalt remains skid-only, and unified snowfall now uses smaller 0.9-2.2 px source flakes with an 8 px shader cap so flakes remain proportional to the car.',
    '2026-08-24 08:35 EDT - Unified race snowfall into one bounded terrain-aware near-to-far field. The duplicate 128-flake car-local canvas overlay no longer updates or renders over the 384-flake environment field; the canonical field now reaches the near camera region and carries the preferred larger flakes and speed streaks through one selected WebGL or canvas backend per frame.',
    '2026-08-23 14:27 EDT - Fixed intermittent hill resets that faded back in over an unchanged overturned car. The fade transaction now treats a rejected static-support solve as uncommitted, remains in the hidden interval, and retries the full reset; each retry advances a fresh terrain-query generation so stale crash-pose hill geometry cannot be reused.',
    '2026-08-22 12:04 EDT - Tightened authoritative and worker-provisional reset orientation validity so a WRX resting nearly on its side can no longer be accepted as already upright. Held Select now rebuilds a genuinely wheel-side-down route-aligned pose and the static support transaction must finish with all four wheels supported on the flat reset regression.',
  {
    date: '2026-08-21',
    time: '01:35 EDT',
  ...
  },
  {
    date: '2026-07-24',
    time: '12:18 EDT',
    title: 'Race terrain formations use complete stable coverage',
    details: [
      'Dense authored heightmaps now render as a complete ten-meter grid over sparse five-meter terrain edits, so absent/default cells and the edges of terrain formations cannot become missing triangles.',
      'The road, margin, shoulder, and transition remain authoritative over the continuous terrain underlay, and physics samples the same baked surface priority instead of falling through to an unrelated heightmap result.',
      'The Three renderer now reuses the baked terrain array and static render key directly instead of cloning tens of thousands of cell references every frame.',
      'Validation distinguishes the permitted world-domain perimeter from internal terrain holes and includes internal coverage failures in the magenta error count.',
      'Added regression coverage for implicit default-ground cells, complete-grid resolution, internal-open-edge rejection, and the existing gravel-hill/contact behavior.'
    ]
  },
  {
    date: '2026-07-11',
...
export function formatLatestChanges(limit = LATEST_CHANGES.length) {
  ...
  return [
    ...
    '',
    'Current Status and Completion Estimate',
    ...LATEST_MAJOR_WORK.currentStatus.map((item) => `- ${item}`),
    '',
...
export function openLatestChangesOverlay({ limit = LATEST_CHANGES.length } = {}) {
  ...
  addSummarySection('Current Status and Completion Estimate', LATEST_MAJOR_WORK.currentStatus);
