...
export default class MidiComposer {
  constructor(game) {
    ...
    this.menuItems = [
      ...
      { id: 'tempo-up', label: 'Tempo +' },
      { id: 'tempo-down', label: 'Tempo -' },
      { id: 'instrument-add', label: 'Add Instrument' },
      { id: 'instrument-remove', label: 'Remove Instrument' },
      { id: 'loop-bar', label: 'Loop Bar' },
      { id: 'play-toggle', label: 'Play / Stop' },
  ...
  update(input) {
    ...
    if (this.focus === 'instruments') {
      ...
      if (up) this.menuIndex = (this.menuIndex - 1 + this.menuItems.length) % this.menuItems.length;
      if (down) this.menuIndex = (this.menuIndex + 1) % this.menuItems.length;
      ...
      if (up) this.cursor.row = clamp(this.cursor.row - 1, 0, this.rows - 1);
  ...
  handleMenuAction(action) {
    ...
    if (action === 'instrument-add') {
      const available = INSTRUMENT_LIBRARY.filter(
        (name) => !this.activeInstruments.some((instrument) => instrument.name === name)
      );
      const nextName = available[0] || `Instrument ${this.activeInstruments.length + 1}`;
      this.activeInstruments.push({ id: randomId(), name: nextName, notes: buildGrid(this.rows, this.cols) });
  ...
  draw(ctx, width, height) {
    ...
    ctx.font = '14px Courier New';
    ctx.fillText(`Tempo: ${song?.tempo || 120} BPM`, topX + 20, panelY + 56);
    const mood = MOOD_PRESETS[song?.moodIndex || 0];
    ctx.fillText(`Style: ${mood?.label || 'Rock'}`, topX + 20, panelY + 80);
    ctx.fillText(`Playback: ${this.isPlaying ? 'Playing' : 'Stopped'}`, topX + 260, panelY + 32);
    ctx.fillText(`Progression: ${song?.progression?.join(' - ') || 'I - V - vi - IV'}`, topX + 260, panelY + 56);
    ...
    if (this.loopEnabled) {
      ...
      ctx.fillRect(loopX, gridY, this.colsPerBar * cellSize, actualH);
      ctx.strokeStyle = '#ffe16a';
      ctx.strokeRect(loopX, gridY, this.colsPerBar * cellSize, actualH);
    }
    ...
    ctx.fillText('D-pad: navigate • Enter: toggle notes/menu • Esc: back', 24, height - 14);
