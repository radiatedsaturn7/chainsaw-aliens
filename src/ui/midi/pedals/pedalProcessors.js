...
const processors = {
  octave: ({ notes, pedal }) => {
    ...
    const tight = clamp(pedal.knobs.tightness ?? 0, 0, 1);
    notes.forEach((note, idx) => {
      const nudge = Math.round((1 - tight) * ((idx % 2 === 0 ? -1 : 1) * 3));
  ...
  compressor: ({ notes, pedal }) => ({ notes: notes.map((note, idx) => {
    ...
    const punch = clamp(pedal.knobs.punch ?? 0.2, 0, 1);
  ...
  wah: ({ notes, cc, pedal, songSettings }) => {
    ...
    const q = clamp(pedal.knobs.q ?? 0.5, 0, 1);
  ...
  chorus: ({ notes, pedal }) => {
    ...
    const drift = clamp(pedal.knobs.drift ?? 0.3, 0, 1);
  ...
  reverb: ({ notes, cc, pedal }) => {
    ...
    const dampen = clamp(pedal.knobs.dampen ?? 0.4, 0, 1);
    const mix = clamp(pedal.knobs.mix ?? 0.5, 0, 1);
    cc.push({ tick: 0, controller: 91, value: Math.round(clamp(room * 0.6 + decay * 0.3 + mix * 0.3, 0, 1) * 127) });
    cc.push({ tick: 0, controller: 71, value: Math.round(clamp(0.5 + (1 - dampen) * 0.35, 0, 1) * 127) });
    return { notes, cc };
  },
