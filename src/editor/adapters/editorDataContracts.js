...
export function ensurePixelPreviewFrame(pixelData, frameIndex = 0) {
  ...
  const buildEditorComposite = () => {
    ...
    if (!composite.some((value) => value)) return null;
    pixelData.size = size;
    pixelData.frames = [composite];
    return composite;
  };
  const editorComposite = buildEditorComposite();
  if (editorComposite) return editorComposite;
