...
function packTerrainSample(sample = {}) {
  return [
    ...
    [sample.physics?.valid === false ? null : q(sample.physics?.heightM),
      ...
      sample.physics?.reason || null, sample.physics?.supportFamilyId ?? null,
      sample.physics?.supportFamilyDriveable === true ? 1 : 0,
      sample.physics?.supportEdgeClassification || null],
...
function unpackTerrainSample(sample = []) {
  return {
    ...
    physics: { heightM: sample[4]?.[0], normal: vectorObject(sample[4]?.[1]),
      ...
      reason: sample[4]?.[8] || null, supportFamilyId: sample[4]?.[9] ?? null,
      supportFamilyDriveable: sample[4]?.[10] === 1,
      supportEdgeClassification: sample[4]?.[11] || null },
  ...
  return [
      ...
      point.contactType, vectorArray(point.worldPoint)]),
    {
      contacts: frame.body.contacts.map((contact) => [
        ...
        contact.supportFamilyDriveable === true ? 1 : 0,
        contact.supportEdgeClassification || null,
        contact.manifoldClusterKey || null
      ]),
  ...
  return {
    ...
    body: {
      contacts: (body.contacts || []).map((contact) => ({
        ...
        sweepMechanism: contact[26], supportFamilyId: contact[27] ?? null,
        supportFamilyDriveable: contact[28] === 1,
        supportEdgeClassification: contact[29] || null,
        manifoldClusterKey: contact[30] || null
      })),
