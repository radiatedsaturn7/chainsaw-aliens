...
test('save-as is prefilled after import so clicking Save persists document', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    window.__pendingSaveAs = studio.saveArtDocument({ forceSaveAs: true });
  });
  ...
  await page.waitForFunction(async () => {
    if (!window.__pendingSaveAs) return false;
    const result = await window.__pendingSaveAs;
